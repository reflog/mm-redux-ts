...
export function buildDisplayableChannelList(
  ...
  const missingDirectChannels = createMissingDirectChannels(
  ...
  const favoriteChannels = buildFavoriteChannels(
    channels,
    myPreferences,
    locale
  );
  const notFavoriteChannels = buildNotFavoriteChannels(channels, myPreferences);
  const directAndGroupChannels = buildDirectAndGroupChannels(
    notFavoriteChannels,
    myMembers,
    config,
    myPreferences,
  ...
  return {
    ...
    privateChannels: notFavoriteChannels.filter(isPrivateChannel) as Array<
      Channel
    >,
    directAndGroupChannels,
  };
}
export function buildDisplayableChannelListWithUnreadSection(
  usersState: UsersState,
  ...
  const unreadChannels = [
    ...buildChannelsWithMentions(channels, myMembers, locale),
    ...buildUnreadChannels(channels, myMembers, locale),
  ];
  const notUnreadChannels = channels.filter(
    (channel: Channel) => !isUnreadChannel(myMembers, channel)
  );
  const favoriteChannels = buildFavoriteChannels(
    notUnreadChannels,
    myPreferences,
  ...
  const notFavoriteChannels = buildNotFavoriteChannels(
    notUnreadChannels,
    myPreferences
  );
  const directAndGroupChannels = buildDirectAndGroupChannels(
...
export function isAutoClosed(
  ...
  const viewTimePref =
    myPreferences[
      `${Preferences.CATEGORY_CHANNEL_APPROXIMATE_VIEW_TIME}--${channel.id}`
    ];
  ...
  if (
    ...
    isFavoriteChannel(myPreferences, channel.id!)
  ) {
    return false;
  }
  ...
  const autoClose =
    myPreferences[
      `${Preferences.CATEGORY_SIDEBAR_SETTINGS}--close_unused_direct_messages`
    ];
...
export function isDirectChannelVisible(
  ...
  const otherUser =
    typeof otherUserOrOtherUserId === 'object' ? otherUserOrOtherUserId : null;
  const otherUserId =
    typeof otherUserOrOtherUserId === 'object'
      ? otherUserOrOtherUserId.id
      : otherUserOrOtherUserId;
  const dm =
    myPreferences[
      `${Preferences.CATEGORY_DIRECT_CHANNEL_SHOW}--${otherUserId}`
    ];
...
export function isGroupChannelVisible(
  ...
  const gm =
    myPreferences[`${Preferences.CATEGORY_GROUP_CHANNEL_SHOW}--${channel.id}`];
...
export function showCreateOption(
  ...
  if (channelType === General.OPEN_CHANNEL) {
    if (
      config.RestrictPublicChannelCreation === General.SYSTEM_ADMIN_ROLE &&
      !isSystemAdmin
    ) {
      return false;
    } else if (
      config.RestrictPublicChannelCreation === General.TEAM_ADMIN_ROLE &&
      !isAdmin
    ) {
      return false;
    }
    ...
    if (
      config.RestrictPrivateChannelCreation === General.SYSTEM_ADMIN_ROLE &&
      !isSystemAdmin
    ) {
      return false;
    } else if (
      config.RestrictPrivateChannelCreation === General.TEAM_ADMIN_ROLE &&
      !isAdmin
    ) {
      return false;
    }
...
export function showManagementOptions(
  ...
  if (channel.type === General.OPEN_CHANNEL) {
    if (
      config.RestrictPublicChannelManagement === General.SYSTEM_ADMIN_ROLE &&
      !isSystemAdmin
    ) {
      return false;
    }
    ...
    if (
      config.RestrictPublicChannelManagement === General.TEAM_ADMIN_ROLE &&
      !isAdmin
    ) {
      return false;
    }
    ...
    if (
      config.RestrictPublicChannelManagement === General.CHANNEL_ADMIN_ROLE &&
      !isChannelAdmin &&
      !isAdmin
    ) {
      return false;
    ...
    if (
      config.RestrictPrivateChannelManagement === General.SYSTEM_ADMIN_ROLE &&
      !isSystemAdmin
    ) {
      return false;
    }
    ...
    if (
      config.RestrictPrivateChannelManagement === General.TEAM_ADMIN_ROLE &&
      !isAdmin
    ) {
      return false;
    }
    ...
    if (
      config.RestrictPrivateChannelManagement === General.CHANNEL_ADMIN_ROLE &&
      !isChannelAdmin &&
      !isAdmin
    ) {
      return false;
...
export function showDeleteOption(
  ...
  if (channel.type === General.OPEN_CHANNEL) {
    if (
      config.RestrictPublicChannelDeletion === General.SYSTEM_ADMIN_ROLE &&
      !isSystemAdmin
    ) {
      return false;
    }
    ...
    if (
      config.RestrictPublicChannelDeletion === General.TEAM_ADMIN_ROLE &&
      !isAdmin
    ) {
      return false;
    }
    ...
    if (
      config.RestrictPublicChannelDeletion === General.CHANNEL_ADMIN_ROLE &&
      !isChannelAdmin &&
      !isAdmin
    ) {
      return false;
    ...
    if (
      config.RestrictPrivateChannelDeletion === General.SYSTEM_ADMIN_ROLE &&
      !isSystemAdmin
    ) {
      return false;
    }
    ...
    if (
      config.RestrictPrivateChannelDeletion === General.TEAM_ADMIN_ROLE &&
      !isAdmin
    ) {
      return false;
    }
    ...
    if (
      config.RestrictPrivateChannelDeletion === General.CHANNEL_ADMIN_ROLE &&
      !isChannelAdmin &&
      !isAdmin
    ) {
      return false;
...
export function canManageMembersOldPermissions(
  channel: Partial<Channel>,
  user: Partial<UserProfile> & { roles },
  teamMember: Partial<TeamMembership> & { roles },
  channelMember: Partial<ChannelMembership> & { roles },
  config: any,
  license: any
): boolean {
  if (
  ...
  if (channel.type === General.PRIVATE_CHANNEL) {
    ...
    if (
      config.RestrictPrivateChannelManageMembers ===
        General.PERMISSIONS_SYSTEM_ADMIN &&
      !isSystemAdmin
    ) {
      return false;
    }
    ...
    if (
      config.RestrictPrivateChannelManageMembers ===
        General.PERMISSIONS_TEAM_ADMIN &&
      !isTeamAdmin &&
      !isSystemAdmin
    ) {
      return false;
    ...
    const isChannelAdmin = channelMember.roles.includes(
...
export function getChannelsIdForTeam(
  ...
  return Object.keys(channels)
    ...
    .reduce<string[]>((res, channel: Channel) => {
...
function createMissingDirectChannels(
  currentUserId: string,
  allChannels: Array<Channel>,
  myPreferences: {
    [x: string]: PreferenceType;
  ...
  const directChannelsDisplayPreferences = getPreferencesByCategory(
    myPreferences,
  ...
  return Array.from(directChannelsDisplayPreferences)
    ...
    .filter(
      teammateId =>
        !allChannels.some(
          isDirectChannelForUser.bind(null, currentUserId, teammateId)
        )
    )
...
function isDirectChannelForUser(userId, otherUserId, channel) {
  return (
    channel.type === General.DM_CHANNEL &&
    getUserIdFromChannelName(userId, channel.name) === otherUserId
  );
}
...
function channelHasUnreadMessages(
  ...
  if (member) {
    ...
    const onlyMentions =
      member.notify_props &&
      member.notify_props.mark_unread === General.MENTION;
...
export function isUnreadChannel(
  members: RelationOneToOne<Channel, ChannelMembership>,
  channel: Channel
): boolean {
  const member = members[channel.id];
...
export function isChannelMuted(member: ChannelMembership): boolean {
  return member && member.notify_props
...
export function areChannelMentionsIgnored(
  ...
  let ignoreChannelMentions = channelMemberNotifyProps.ignore_channel_mentions as string;
...
function buildChannels(
  ...
  missingDirectChannels: Array<Channel>,
  teammateNameDisplay: string,
  locale: string
): Array<Channel> {
  return channels
    .concat(missingDirectChannels)
    .map(completeDirectChannelInfo.bind(
      null,
      usersState,
      teammateNameDisplay
    ) as () => Channel)
...
function buildFavoriteChannels(
  ...
  return channels
    .filter(channel => isFavoriteChannel(myPreferences, channel.id))
    .sort(sortChannelsByDisplayName.bind(null, locale));
}

function buildNotFavoriteChannels(
  channels: Array<Channel>,
  myPreferences: {
    [x: string]: PreferenceType;
  }
  ...
  return channels.filter(
    channel => !isFavoriteChannel(myPreferences, channel.id)
  );
}

function buildDirectAndGroupChannels(
  channels: Array<Channel>,
  memberships: RelationOneToOne<Channel, ChannelMembership>,
  config: any,
  myPreferences: {
...
function buildChannelsWithMentions(channels, members, locale) {
  return channels
    .filter(channelHasMentions.bind(null, members))
    .sort(sortChannelsByDisplayName.bind(null, locale));
}

function buildUnreadChannels(channels, members, locale) {
  return channels
    .filter(channelHasUnreadMessages.bind(null, members))
    .sort(sortChannelsByDisplayName.bind(null, locale));
}
