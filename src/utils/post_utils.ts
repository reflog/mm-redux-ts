...
import { PreferenceType } from 'types/preferences.js';
import { Post, PostType } from 'types/posts.js';
...
export function isSystemMessage(post: Partial<Post>): boolean {
  return Boolean(
    post.type && post.type.startsWith(Posts.SYSTEM_MESSAGE_PREFIX)
  );
}
export function isMeMessage(post: Partial<Post>): boolean {
  return Boolean(post.type && post.type === Posts.POST_TYPES.ME);
}
export function isFromWebhook(post: Partial<Post>): boolean {
  return post.props && post.props.from_webhook;
}
export function isPostEphemeral(post: Partial<Post>): boolean {
  return (
...
export function shouldIgnorePost(post: Partial<Post>): boolean {
...
export function isPostOwner(userId: $ID<UserProfile>, post: Partial<Post>) {
  return userId === post.user_id;
}
export function isEdited(post: Partial<Post>): boolean {
...
export function canDeletePost(
  ...
  if (license.IsLicensed === 'true') {
    return (
      (config.RestrictPostDelete === General.PERMISSIONS_ALL &&
        (isOwner || isAdmin)) ||
      (config.RestrictPostDelete === General.PERMISSIONS_TEAM_ADMIN &&
        isAdmin) ||
      (config.RestrictPostDelete === General.PERMISSIONS_SYSTEM_ADMIN &&
        isSystemAdmin)
    );
  }
