...
export function getFullName(user: Partial<UserProfile>): string {
...
export function displayUsername(
  user: Partial<UserProfile> & { username },
...
export function rolesIncludePermission(
  roles: string,
  permission: string
): boolean {
...
export function profileListToMap(
  profileList: Array<Partial<UserProfile>>
...
export function getSuggestionsSplitBy(
  ...
  const initialSuggestions = splitTerm.map((_, i) =>
...
export function getSuggestionsSplitByMultiple(
  term: string,
  splitStrs: Array<string>
): Array<string> {
  const suggestions = splitStrs.reduce<Set<string>>((acc, val) => {
    getSuggestionsSplitBy(term, val).forEach(suggestion => acc.add(suggestion));
    return acc;
  }, new Set());
  return [...suggestions];
}
export function filterProfilesMatchingTerm(
  users: Array<Partial<UserProfile>>,
  term: string
  ...
  return users.filter((user: Partial<UserProfile>) => {
    if (!user) {
      return false;
    }
