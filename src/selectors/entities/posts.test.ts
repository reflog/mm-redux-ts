...
describe('Selectors.Posts', () => {
  ...
  it('should return profiles for reactions', () => {
    ...
    assert.deepEqual(getProfilesForReactions(testState, [reaction1 as any]), [
      user1,
    ]);
  });
  it('get posts in channel', () => {
    ...
    assert.deepEqual(getPostsInChannel(testState, '1', 30), [
      post5,
      post4,
      post3,
      post2,
  ...
  it('get posts around post in channel', () => {
    ...
    assert.deepEqual(getPostsAroundPost(testState, post3.id, '1'), [
      post5,
      post4,
      post3,
      post2,
  ...
  it('get posts in channel with notify comments as any', () => {
    ...
    const getPostsInChannel = Selectors.makeGetPostsInChannel();
    assert.deepEqual(getPostsInChannel(testStateAny, '1'), [
      post6,
      post5,
      post4,
      post3,
  ...
  it('get posts in channel with notify comments as root', () => {
    ...
    const getPostsInChannel = Selectors.makeGetPostsInChannel();
    assert.deepEqual(getPostsInChannel(testStateRoot, '1'), [
      post6,
      post5,
      post4,
      post3,
  ...
  it('get posts in channel with notify comments as never', () => {
    ...
    const getPostsInChannel = Selectors.makeGetPostsInChannel();
    assert.deepEqual(getPostsInChannel(testStateNever, '1'), [
      post6,
      post5,
      post4,
      post3,
  ...
  it('gets posts around post in channel not adding ephemeral post to replyCount', () => {
    ...
    const getPostsAroundPost = Selectors.makeGetPostsAroundPost();
    assert.deepEqual(getPostsAroundPost(testStateAny, post1.id, '1'), [
      post3,
      post2,
      post1,
    ]);
  ...
  describe('getPostIdsForThread', () => {
    ...
    it('memoization', () => {
      ...
      assert.equal(now, previous); // One post, unrelated changes
      ...
      assert.deepEqual(now, ['1005']);
      assert.equal(now, previous); // One post, changes to post
      ...
      previous = now;
      now = getPostIdsForThread(state as any, '1005');
      assert.deepEqual(now, ['1005']);
      assert.equal(now, previous); // Change of thread

      previous = now;
      now = getPostIdsForThread(state as any, '1001');
      assert.deepEqual(now, ['1004', '1002', '1001']);
      assert.notEqual(now, previous);
      previous = now;
      now = getPostIdsForThread(state as any, '1001');
      assert.equal(now, previous); // New post in thread
  ...
  describe('getPostIdsAroundPost', () => {
    ...
    it('posts around', () => {
      ...
      assert.deepEqual(getPostIdsAroundPost(state, 'c', '1234'), [
        'a',
        'b',
        'c',
        'd',
    ...
    it('memoization', () => {
      ...
      assert.equal(now, previous); // Changes to posts in another channel
      ...
      assert.deepEqual(now, ['a', 'b', 'c', 'd', 'e']);
      assert.equal(now, previous); // Changes to posts in this channel
      ...
      now = getPostIdsAroundPost(state, 'c', '1234');
      assert.deepEqual(now, ['a', 'b', 'c', 'd', 'e', 'f']);
      assert.equal(now, previous); // Change of channel
      ...
      now = getPostIdsAroundPost(state, 'i', 'abcd');
      assert.deepEqual(now, ['g', 'h', 'i', 'j', 'k', 'l']);
      assert.equal(now, previous); // With limits
      ...
      assert.deepEqual(now, ['h', 'i', 'j', 'k']);
      assert.equal(now, previous); // Change of limits
      ...
      assert.deepEqual(now, ['g', 'h', 'i', 'j']);
      assert.equal(now, previous); // Change of post
      ...
      assert.deepEqual(now, ['h', 'i', 'j', 'k']);
      assert.equal(now, previous); // Change of posts past limit
      ...
      assert.deepEqual(now, ['h', 'i', 'j', 'k']);
      assert.equal(now, previous); // Change of post order
  ...
  describe('getPostsForIds', () => {
    ...
    it('memoization', () => {
      ...
      assert.equal(now, previous); // New, identical ids
      ...
      assert.equal(now, previous); // New posts, no changes to ones in ids
      ...
      assert.equal(now, previous); // New ids
      ...
      assert.equal(now, previous); // New posts, changes to ones in ids
      ...
      state = {
        ...
        entities: {
          ...
          posts: {
            ...state.entities.posts,
            posts: { ...state.entities.posts.posts, [newPost.id]: newPost },
          },
        },
      };
      previous = now;
      ...
      const modifiedThreadState = {
        ...
        entities: {
          ...
          posts: {
            ...
            posts: {
              ...modifiedState.entities.posts.posts,
              a: { ...modifiedState.entities.posts.posts.a, user_id: 'b' },
              c: { ...modifiedState.entities.posts.posts.c, user_id: user1.id },
            },
          },
        },
      };
