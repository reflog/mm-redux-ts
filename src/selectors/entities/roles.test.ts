...
describe('Selectors.Roles', () => {
  ...
  myChannelMembers[channel1.id] = {
    roles: 'test_channel_a_role1 test_channel_a_role2',
  };
  myChannelMembers[channel2.id] = {
    roles: 'test_channel_a_role1 test_channel_a_role2',
  };
  myChannelMembers[channel3.id] = {
    roles: 'test_channel_a_role1 test_channel_a_role2',
  };
  myChannelMembers[channel4.id] = {
    roles: 'test_channel_a_role1 test_channel_a_role2',
  };
  myChannelMembers[channel5.id] = {
    roles: 'test_channel_a_role1 test_channel_a_role2',
  };
  myChannelMembers[channel7.id] = {
    roles: 'test_channel_b_role1 test_channel_b_role2',
  };
  myChannelMembers[channel8.id] = {
    roles: 'test_channel_b_role1 test_channel_b_role2',
  };
  myChannelMembers[channel9.id] = {
    roles: 'test_channel_b_role1 test_channel_b_role2',
  };
  myChannelMembers[channel10.id] = {
    roles: 'test_channel_c_role1 test_channel_c_role2',
  };
  myChannelMembers[channel11.id] = {
    roles: 'test_channel_c_role1 test_channel_c_role2',
  };
  myChannelMembers[channel12.id] = {};
  const roles = {
    test_team1_role1: {
      permissions: ['team1_role1'],
    },
    test_team2_role1: {
      permissions: ['team2_role1'],
    },
    test_team2_role2: {
      permissions: ['team2_role2'],
    },
    test_channel_a_role1: {
      permissions: ['channel_a_role1'],
    },
    test_channel_a_role2: {
      permissions: ['channel_a_role2'],
    },
    test_channel_b_role2: {
      permissions: ['channel_b_role2'],
    },
    test_channel_c_role1: {
      permissions: ['channel_c_role1'],
    },
    test_channel_c_role2: {
      permissions: ['channel_c_role2'],
    },
    test_user_role2: {
      permissions: ['user_role2'],
    },
  };
  ...
  it('should return my system permission on getMySystemPermissions', () => {
    assert.deepEqual(
      Selectors.getMySystemPermissions(testState),
      new Set(['user_role2'])
    );
  });
  ...
  it('should return my team permission on getMyTeamPermissions', () => {
    assert.deepEqual(
      Selectors.getMyTeamPermissions(testState, {
        team: team1.id,
      }),
  ...
  it('should return if i have a team permission on haveITeamPermission', () => {
    assert.equal(
      Selectors.haveITeamPermission(testState, {
        team: team1.id,
        permission: 'user_role2',
      }),
      true
  ...
  it('should return my team permission on getMyCurrentTeamPermissions', () => {
    assert.deepEqual(
      Selectors.getMyCurrentTeamPermissions(testState),
      new Set(['user_role2', 'team1_role1'])
    );
  });
  it('should return if i have a team permission on haveICurrentTeamPermission', () => {
    assert.equal(
      Selectors.haveICurrentTeamPermission(testState, {
        permission: 'user_role2',
      }),
      true
    );
  ...
  it('should return my channel permission on getMyChannelPermissions', () => {
    assert.deepEqual(
      Selectors.getMyChannelPermissions(testState, {
        team: team1.id,
        channel: channel1.id,
      }),
      new Set([
  ...
  it('should return if i have a channel permission on haveICurrentChannelPermission', () => {
    assert.equal(
      Selectors.haveICurrentChannelPermission(testState, {
        permission: 'user_role2',
      }),
      true
    );
