...
import {
  ...
  getLicense,
  hasNewPermissions,
} from 'selectors/entities/general';
import {
...
export type SortingType = 'recent' | 'alpha';
export type GroupingType = 'by_type' | 'none';
...
export const getDirectChannelsSet: (
  a: GlobalState
...
export const mapAndSortChannelIds = (
  ...
  sorting: SortingType,
  sortMentionsFirst: boolean = false
): Array<string> => {
  ...
  const otherChannelIds = channels
    .filter(channel => {
      return (
        !mutedChannelIds.includes(channel.id) &&
        !hasMentionedChannelIds.includes(channel.id)
      );
    })
    .sort(sortChannelsByRecencyOrAlpha.bind(null, locale, lastPosts, sorting))
    .map(channel => channel.id);
...
export const getCurrentChannel: (a: GlobalState) => Channel = createSelector(
  getAllChannels,
  getCurrentChannelId,
  (state: GlobalState): UsersState => state.entities.users,
  getTeammateNameDisplaySetting,
...
export const getCurrentChannelStats: (
  a: GlobalState
) => ChannelStats = createSelector(
  getAllChannelStats,
  getCurrentChannelId,
  (
...
export const isCurrentChannelFavorite: (
  a: GlobalState
  ...
  (
    ...
    channelId: string
  ): boolean => isFavoriteChannel(preferences, channelId)
);
export const isCurrentChannelMuted: (
  a: GlobalState
) => boolean = createSelector(
  getMyCurrentChannelMembership,
  (membership: ChannelMembership | undefined | null): boolean => {
    if (!membership) {
      return false;
    }
...
export const isCurrentChannelArchived: (
  a: GlobalState
) => boolean = createSelector(
  getCurrentChannel,
  (channel: Channel): boolean => channel.delete_at !== 0
);
export const isCurrentChannelDefault: (
  a: GlobalState
) => boolean = createSelector(
  getCurrentChannel,
...
export function shouldHideDefaultChannel(
  state: GlobalState,
  channel: Channel
): boolean {
  return (
...
export const getChannelsInCurrentTeam: (
  a: GlobalState
) => Array<Channel> = createSelector(
  getAllChannels,
  getChannelSetInCurrentTeam,
  getCurrentUser,
  (
    channels: IDMappedObjects<Channel>,
    currentTeamChannelSet: Array<string>,
    currentUser: UserProfile
  ): Array<Channel> => {
...
export const getAllDirectChannels: (
  a: GlobalState
) => Array<Channel> = createSelector(
  getAllChannels,
  ...
  (
    ...
    channelSet.forEach(c => {
      dmChannels.push(
        completeDirectChannelInfo(users, teammateNameDisplay, channels[c])
      );
    });
    return dmChannels;
  }
  ...
  (
    ...
    channelSet.forEach(id => {
      ...
      if (channel.type === General.GM_CHANNEL) {
        gmChannels.push(
          completeDirectChannelInfo(users, teammateNameDisplay, channel)
        );
      }
    });
    return gmChannels;
...
export const getArchivedChannels: (
  ...
  (
    ...
    return channels.filter(
      c => myMembers.hasOwnProperty(c.id) && c.delete_at !== 0
    );
  }
);
export const getChannelsByCategory: (
  ...
  (
    ...
    return buildDisplayableChannelList(
      usersState,
      allChannels,
      myMembers,
      config,
...
export const getChannelsWithUnreadSection: (
  a: GlobalState
) => {
  ...
  (
    ...
    return buildDisplayableChannelListWithUnreadSection(
      usersState,
      allChannels,
      myMembers,
      config,
...
export const getDefaultChannel: (
  a: GlobalState
) => Channel | undefined | null = createSelector(
  getAllChannels,
  getCurrentTeamId,
  (
    channels: IDMappedObjects<Channel>,
    ...
    return Object.keys(channels)
      ...
      .find(
        c => c && c.team_id === teamId && c.name === General.DEFAULT_CHANNEL
      );
  }
);
...
export const getUnreads: (
  ...
  (
    ...
    Object.keys(myMembers).forEach(channelId => {
      ...
      if (
        ...
        if (
          ...
          channel.total_msg_count - m.msg_count > 0
        ) {
          if (channel.type === 'D') {
            if (users[otherUserId] && users[otherUserId].delete_at === 0) {
    ...
    const totalTeamsUnreadCount = {
      messageCount:
        messageCountForCurrentTeam +
        otherTeamsUnreadCountForChannels.messageCount,
      mentionCount:
        mentionCountForCurrentTeam +
        otherTeamsUnreadCountForChannels.mentionCount,
    };
...
export const canManageChannelMembers: (
  ...
  (
    ...
    return canManageMembersOldPermissions(
      channel,
      user,
      teamMembership,
      channelMembership,
...
export const canManageAnyChannelMembersInCurrentTeam: (
  a: GlobalState
) => boolean = createSelector(
  getMyChannelMemberships,
  getCurrentTeamId,
...
export const getAllDirectChannelIds: (
  a: GlobalState
) => Array<string> = createIdsSelector(
  getDirectChannelsSet,
  (directIds: Set<string>): Array<string> => {
    return Array.from(directIds);
  }
);
export const getChannelIdsInCurrentTeam: (
  a: GlobalState
) => Array<string> = createIdsSelector(
  getCurrentTeamId,
  getChannelsInTeam,
...
export const getChannelIdsForCurrentTeam: (
  a: GlobalState
) => Array<string> = createIdsSelector(
...
export const getUnreadChannelIds: (
  ...
  (
    ...
    lastUnreadChannel: Channel | undefined | null
  ): Array<string> => {
...
export const getUnreadChannels: (
  a: GlobalState,
  b?: Channel | undefined | null
) => Array<Channel> = createIdsSelector(
  getCurrentUser,
  getUsers,
...
export const getSortedUnreadChannelIds: (
  ...
  (unreadChannelIds, mappedAndSortedUnreadChannelIds) =>
    ...
    const favoriteChannel = favoriteIds
      .filter(id => {
        ...
        if (
          ...
          channel.type === General.GM_CHANNEL &&
          !isGroupChannelVisible(config, prefs, channel)
        ) {
          return false;
        }
...
export const getSortedPublicChannelIds: (
  ...
  (state, lastUnreadChannel, unreadsAtTop = true) => unreadsAtTop,
  (state, lastUnreadChannel, unreadsAtTop, favoritesAtTop = true) =>
    favoritesAtTop,
  filterChannels
); // Private Channels
  ...
  (
    ...
    teammates.reduce((result, teammateId) => {
      ...
      if (channel) {
        ...
        const otherUser =
          profiles[getUserIdFromChannelName(currentUser.id, channel.name)];
...
export const getDirectAndGroupChannels: (
  a: GlobalState
) => Array<Channel> = createSelector(
  getCurrentUser,
  getUsers,
...
export function getGroupOrDirectChannelVisibility(
  state: GlobalState,
  channelId: string
): boolean {
...
export const filterPostIds = (condition?: (a: Channel, b: Post) => boolean) => {
  ...
  ) as (a: GlobalState, b: Array<string>) => Array<string>;
};

const getProfiles = (
  currentUserId: string,
  usersIdsInChannel: Array<string>,
  users: IDMappedObjects<UserProfile>
): Array<UserProfile> => {
...
const hasChannelsChanged = channels => {
  ...
  for (let i = 0; i < channels.length; i++) {
    if (
      channels[i].type !== lastChannels[i].type ||
      channels[i].items !== lastChannels[i].items
    ) {
      return true;
    }
  }
...
export const getSortedPrivateChannelWithUnreadsIds: (
  a: GlobalState,
  b?: Channel,
  c?: boolean,
  d?: boolean,
...
export const getSortedDirectChannelWithUnreadsIds: (
  a: GlobalState,
  b?: Channel,
  c?: boolean,
  d?: boolean,
