...
describe('Selectors.Groups', () => {
  ...
  it('getGroupsAssociatedToTeam', () => {
    ...
    assert.deepEqual(
      Selectors.getGroupsAssociatedToTeam(testState, teamID),
      expected
    );
  });
  ...
  it('getGroupsNotAssociatedToChannel', () => {
    const expected = Object.entries(testState.entities.groups.groups)
      .filter(([groupID]) => !channelAssociatedGroupIDs.includes(groupID))
      .map(([, group]) => group);
    assert.deepEqual(
      Selectors.getGroupsNotAssociatedToChannel(testState, channelID),
      expected
    );
  });
});
