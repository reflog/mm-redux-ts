...
import { SchemeScope } from 'types/schemes';
describe('Selectors.Schemes', () => {
  const scheme1 = TestHelper.mockSchemeWithId();
  scheme1.scope = ScopeTypes.CHANNEL as SchemeScope;
  const scheme2 = TestHelper.mockSchemeWithId();
  scheme2.scope = ScopeTypes.TEAM as SchemeScope;
  const schemes = {};
  ...
  it('makeGetSchemeChannels', () => {
    ...
    const results = getSchemeChannels(testState, {
      schemeId: scheme1.id,
    });
  ...
  it('makeGetSchemeTeams', () => {
    ...
    const results = getSchemeTeams(testState, {
      schemeId: scheme2.id,
    });
    assert.equal(results.length, 1);
