...
export function get(state, category, name, defaultValue: string | number = '') {
...
export function getFavoritesPreferences(state) {
  const favorites = getFavoritesCategory(
    state,
...
export const getVisibleTeammate = createSelector(
  ...
  direct => {
    return direct
      .filter(dm => dm.value === 'true' && dm.name)
      .map(dm => dm.name);
  }
);
...
export const getTheme = createShallowSelector(
  ...
  (themePreference, defaultTheme) => {
    ...
    if (theme.type && theme.type !== 'custom') {
      const match = Object.values(Preferences.THEMES).find(
        v => v.type === theme.type
      );
...
export const getSidebarPreferences = createSelector(
  ...
  state => {
    return get(state, Preferences.CATEGORY_SIDEBAR_SETTINGS, '');
  },
