...
export function getTeam(state: GlobalState, id) {
...
export const getSortedJoinableTeams = createSelector(
  ...
  getJoinableTeamIds,
  (state, locale) => locale,
  (teams, joinableTeamIds, locale): Team[] => {
    const joinableTeams = {};
    ...
    return Object.values<Team>(joinableTeams).sort(sortTeamsWithLocale(locale));
  }
);
