...
describe('Selectors.Users', () => {
  ...
  it('getUserIdsInChannels', () => {
    assert.deepEqual(
      Selectors.getUserIdsInChannels(testState),
      profilesInChannel
    );
  });
  it('getUserIdsNotInChannels', () => {
    assert.deepEqual(
      Selectors.getUserIdsNotInChannels(testState),
      profilesNotInChannel
    );
  });
  ...
  it('getUserIdsNotInTeams', () => {
    assert.deepEqual(
      Selectors.getUserIdsNotInTeams(testState),
      profilesNotInTeam
    );
  });
  it('getUserIdsWithoutTeam', () => {
    assert.deepEqual(
      Selectors.getUserIdsWithoutTeam(testState),
      profilesWithoutTeam
    );
  });
  it('getUserSessions', () => {
  ...
  describe('getCurrentUserMentionKeys', () => {
    it('at mention', () => {
      ...
      assert.deepEqual(Selectors.getCurrentUserMentionKeys(state as any), [
        {
          key: '@user',
        },
      ]);
  ...
  describe('getProfiles', () => {
    it('getProfiles without filter', () => {
      const users = [user1, user2, user3, user4, user5, user6, user7].sort(
        sortByUsername
      );
      assert.deepEqual(Selectors.getProfiles(testState as any), users);
    });
  ...
  describe('getProfilesInTeam', () => {
    ...
    it('getProfilesInTeam with inactive filter', () => {
      const users = [user2, user7].sort(sortByUsername);
      assert.deepEqual(
        Selectors.getProfilesInTeam(testState, team1.id, { inactive: true }),
        users
      );
      assert.deepEqual(
        Selectors.getProfilesInTeam(testState, 'junk', { inactive: true }),
        []
      );
    });
  ...
  describe('searchProfiles', () => {
    it('searchProfiles without filter', () => {
      assert.deepEqual(Selectors.searchProfiles(testState, user1.username!), [
        user1,
      ]);
      assert.deepEqual(
        Selectors.searchProfiles(
          testState,
      ...
      assert.deepEqual(
        Selectors.searchProfiles(testState, user1.username!, true),
        []
      );
    });
    it('searchProfiles with filters', () => {
      assert.deepEqual(
        Selectors.searchProfiles(testState, user1.username!, false, {
          role: 'system_admin',
        }),
        [user1]
      );
      assert.deepEqual(
        Selectors.searchProfiles(testState, user3.username!, false, {
          role: 'system_admin',
        }),
        []
      );
      ...
      assert.deepEqual(
        Selectors.searchProfiles(testState, user2.username!, false, {
          inactive: true,
        }),
        [user2]
      );
  ...
  it('searchProfilesNotInCurrentChannel', () => {
    assert.deepEqual(
      Selectors.searchProfilesNotInCurrentChannel(testState, user2.username!),
      [user2]
    );
    assert.deepEqual(
  ...
  it('searchProfilesInCurrentTeam', () => {
    ...
    assert.deepEqual(
      Selectors.searchProfilesInCurrentTeam(testState, user1.username!, true),
      []
    );
  });
  describe('searchProfilesInTeam', () => {
    it('searchProfilesInTeam without filter', () => {
      assert.deepEqual(
        Selectors.searchProfilesInTeam(testState, team1.id, user1.username!),
        [user1]
      );
      assert.deepEqual(
  ...
  describe('searchProfilesWithoutTeam', () => {
    it('searchProfilesWithoutTeam without filter', () => {
      assert.deepEqual(
        Selectors.searchProfilesWithoutTeam(testState, user5.username!),
        [user5]
      );
      assert.deepEqual(
        Selectors.searchProfilesWithoutTeam(testState, user5.username!, true),
        [user5]
      );
    });
    it('searchProfilesWithoutTeam with filter', () => {
      assert.deepEqual(
        Selectors.searchProfilesWithoutTeam(testState, user6.username!, false, {
          role: 'system_admin',
        }),
        [user6]
      );
      assert.deepEqual(
        Selectors.searchProfilesWithoutTeam(testState, user5.username!, false, {
          inactive: true,
        }),
        []
      );
  ...
  it('getUserByUsername', () => {
    assert.deepEqual(
      Selectors.getUserByUsername(testState, user1.username!),
      user1
    );
  });
  ...
  it('makeGetProfilesInChannel', () => {
    ...
    assert.deepEqual(getProfilesInChannel(testState, channel2.id, true), [
      user1,
    ]);
      ...
      entities: {
        ...
        users: {
          ...
          profilesInChannel: {
            ...
            [channel1.id]: new Set([
              ...testState.entities.users.profilesInChannel[channel1.id],
              'unknown',
            ]),
          },
        },
  ...
  it('makeGetProfilesNotInChannel', () => {
    ...
    assert.deepEqual(
      getProfilesNotInChannel(testState, channel1.id, true),
      [user3].sort(sortByUsername)
    );
    assert.deepEqual(
      getProfilesNotInChannel(testState, channel1.id),
      [user2, user3].sort(sortByUsername)
    );
    assert.deepEqual(
      getProfilesNotInChannel(testState, channel2.id, true),
      [user4, user5].sort(sortByUsername)
    );
    assert.deepEqual(
      getProfilesNotInChannel(testState, channel2.id),
      [user4, user5].sort(sortByUsername)
    );
  ...
  it('makeGetProfilesByIdsAndUsernames', () => {
    ...
    const testCases = [
      ...
      {
        input: {
          allUserIds: [user1.id, 'nonexistentid'],
        },
        output: [user1],
      },
      {
      ...
      {
        input: {
          allUserIds: ['nonexistentid', user1.id, user2.id],
        },
        output: [user1, user2],
      },
      {
      ...
      {
        input: {
          allUsernames: [user1.username, 'nonexistentid'],
        },
        output: [user1],
      },
      {
        input: {
          allUsernames: [user1.username, user2.username],
        },
        output: [user1, user2],
      },
      {
        input: {
          allUsernames: [user1.username, 'nonexistentid', user2.username],
        },
        output: [user1, user2],
      },
      {
    ...
    testCases.forEach(testCase => {
      assert.deepEqual(
        getProfilesByIdsAndUsernames(testState, testCase.input as any),
        testCase.output
      );
    });
  });
  it('makeGetDisplayName', () => {
    ...
    assert.deepEqual(
      Selectors.makeGetDisplayName()(newTestState, testUser2.id),
      'nick2'
    );
    ...
    newTestState.entities.general = {
      config: {
        TeammateNameDisplay: General.TEAMMATE_NAME_DISPLAY.SHOW_FULLNAME,
      },
    }; // should match username
    ...
    assert.deepEqual(
      Selectors.makeGetDisplayName()(newTestState, 'not_exist_id'),
      'Someone'
    ); // // should match empty string when not using default fallback

    assert.deepEqual(
      Selectors.makeGetDisplayName()(newTestState, 'not_exist_id', false),
      ''
    );
  });
  it('shouldShowTermsOfService', () => {
    const userId = 1234; // Test latest terms not accepted
