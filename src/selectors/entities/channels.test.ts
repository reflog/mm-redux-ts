...
import { GroupingType, SortingType } from 'selectors/entities/channels';
interface TestProfile extends Partial<UserProfile> {
  fakeUserId?: string;
}
interface TestChannel extends Partial<Channel> {
...
describe('Selectors.Channels', () => {
  ...
  const profiles: { [key: string]: TestProfile } = {};
  profiles[user.id] = user;
  profiles[user2.id] = user2;
  profiles[user3.id] = user3;
  profiles.fakeUserId = TestHelper.fakeUserWithId();
  ...
  channel5.type = General.PRIVATE_CHANNEL as ChannelType;
  channel5.display_name = 'Channel 5';
  channel5.last_post_at = Date.now();
  const channel5Del = TestHelper.fakeChannelWithId('');
  channel5Del.type = General.PRIVATE_CHANNEL as ChannelType;
  ...
  channel7.type = General.GM_CHANNEL as ChannelType;
  ...
  membersInChannel[channel2.id][user.id] = {
    channel_id: channel2.id,
    user_id: user.id,
  };
  membersInChannel[channel3.id] = {};
  membersInChannel[channel3.id][user.id] = {
    channel_id: channel3.id,
    user_id: user.id,
  };
  membersInChannel[channel4.id] = {};
  membersInChannel[channel4.id][user.id] = {
    channel_id: channel4.id,
    user_id: user.id,
  };
  membersInChannel[channel5.id] = {};
  membersInChannel[channel5.id][user.id] = {
    channel_id: channel5.id,
    user_id: user.id,
  };
  membersInChannel[channel5Del.id] = {};
  membersInChannel[channel5Del.id][user.id] = {
    channel_id: channel5Del.id,
    user_id: user.id,
  };
  membersInChannel[channel6.id] = {};
  membersInChannel[channel6Del.id] = {};
  membersInChannel[channel7.id] = {};
  membersInChannel[channel7.id][user.id] = {
    channel_id: channel7.id,
    user_id: user.id,
  };
  membersInChannel[channel7.id][user2.id] = {
    channel_id: channel7.id,
    user_id: user2.id,
  };
  membersInChannel[channel7.id][user3.id] = {
    channel_id: channel7.id,
    user_id: user3.id,
  };
  membersInChannel[channel8.id] = {};
  membersInChannel[channel8.id][user.id] = {
    channel_id: channel8.id,
    user_id: user.id,
  };
  membersInChannel[channel9.id] = {};
  membersInChannel[channel9.id][user.id] = {
    channel_id: channel9.id,
    user_id: user.id,
  };
  membersInChannel[channel10.id] = {};
  membersInChannel[channel10.id][user.id] = {
    channel_id: channel10.id,
    user_id: user.id,
  };
  membersInChannel[channel11.id] = {};
  membersInChannel[channel11.id][user.id] = {
    channel_id: channel11.id,
    user_id: user.id,
  };
  membersInChannel[channel12.id] = {};
  membersInChannel[channel12.id][user.id] = {
    channel_id: channel12.id,
    user_id: user.id,
  };
  membersInChannel[channel12.id][user2.id] = {
    channel_id: channel12.id,
    user_id: user2.id,
  };
  membersInChannel[channel13.id!] = {};
  membersInChannel[channel13.id!][user.id] = {
    channel_id: channel13.id,
    user_id: user.id,
  };
  membersInChannel[channel13.id!][user2.id] = {
    channel_id: channel13.id,
    user_id: user2.id,
  };
  const myMembers = {};
  ...
  myMembers[channel8.id] = {
    channel_id: channel8.id,
    user_id: user.id,
    msg_count: 0,
    notify_props: {},
  };
  myMembers[channel9.id] = {
    channel_id: channel9.id,
    user_id: user.id,
  };
  myMembers[channel10.id] = {
    channel_id: channel10.id,
    user_id: user.id,
  };
  myMembers[channel11.id] = {
    channel_id: channel11.id,
    user_id: user.id,
  };
  myMembers[channel12.id] = {
    channel_id: channel12.id,
    user_id: user.id,
    msg_count: 0,
    notifyProps: {},
  };
  myMembers[channel13.id!] = {
    channel_id: channel13.id,
    user_id: user.id,
    msg_count: 1,
    notifyProps: {},
  ...
  const sortUsernames = (a, b) =>
    a.localeCompare(b, General.DEFAULT_LOCALE, {
      numeric: true,
    });

  it('should return channels in current team', () => {
    const channelsInCurrentTeam = [
      channel1,
      channel2,
      channel5,
      channel5Del,
      ...
      channel10,
      channel11,
    ].sort(sortChannelsByDisplayName.bind(null, []));
    assert.deepEqual(
      Selectors.getChannelsInCurrentTeam(testState),
  ...
  it('should return members in current channel', () => {
    assert.deepEqual(
      Selectors.getMembersInCurrentChannel(testState),
      membersInChannel[channel1.id]
    );
  });
  it('get public channels not member of', () => {
    assert.deepEqual(
      ...
      [channel6, channel6Del].sort(sortChannelsByDisplayName.bind(null, []))
    );
  });
  ...
  it('get first channel that matches by name', () => {
    assert.deepEqual(
      Selectors.getChannelByName(testState, channel1.name!),
      channel1
    );
  });
  ...
  it('get unreads', () => {
    assert.deepEqual(Selectors.getUnreads(testState), {
      messageCount: 4,
      mentionCount: 4,
    });
  });
  it('get unreads with a missing profile entity', () => {
    const newProfiles = { ...testState.entities.users.profiles };
    Reflect.deleteProperty(newProfiles, 'fakeUserId');
    const newState = {
      ...testState,
      entities: {
        ...testState.entities,
        users: { ...testState.entities.users, profiles: newProfiles },
      },
    };
    assert.deepEqual(Selectors.getUnreads(newState), {
      messageCount: 4,
      mentionCount: 2,
    });
    assert.deepEqual(Selectors.getUnreadsInCurrentTeam(newState), {
      messageCount: 3,
      mentionCount: 1,
    });
  });
  ...
  it('get unreads with a deactivated channel', () => {
    ...
    const newState = {
      ...
      entities: {
        ...testState.entities,
        channels: { ...testState.entities.channels, channels: newChannels },
      },
    };
    assert.deepEqual(Selectors.getUnreads(newState), {
      messageCount: 3,
  ...
  it('get channel map for team', () => {
    ...
    assert.deepEqual(
      Selectors.getChannelsNameMapInTeam(testState, team1.id),
      channelMap
    );
  ...
  it('get direct channel ids strict equal', () => {
    ...
    assert.ok(fromOriginalState === fromModifiedState); // it should't have a channel that belongs to a team

    assert.equal(
      fromModifiedState.includes(channel1.id),
      false,
      'should not have a channel that belongs to a team'
    );
  });
  it('get channel ids in current team strict equal', () => {
    ...
    const fromModifiedState = Selectors.getChannelIdsInCurrentTeam(
      modifiedState
    );
    assert.ok(fromOriginalState === fromModifiedState); // it should't have a direct channel

    assert.equal(
      fromModifiedState.includes(channel7.id),
      false,
      'should not have direct channel on a team'
    );
  });
  it('get channel ids for current team strict equal', () => {
    ...
    const fromModifiedState = Selectors.getChannelIdsForCurrentTeam(
      modifiedState
    );
    assert.ok(fromOriginalState === fromModifiedState); // it should have a direct channel
  ...
  it('get sorted unread channel ids in current team strict equal', () => {
    ...
    const fromModifiedState = Selectors.getSortedUnreadChannelIds(
      modifiedState
    );
    const fromMentionState = Selectors.getSortedUnreadChannelIds(mentionState); // mentions should be prioritized to the top

    assert.ok(fromOriginalState === fromModifiedState);
    assert.ok(fromMentionState !== fromModifiedState); // channel8 and channel2 are above all others
    // since default order is "alpha", channel8 with display_name "ABC" should come first

    assert.ok(fromMentionState[0] === channel8.id); // followed by channel2 with display_name "DEF"
    ...
    const fromHasMentionMutedChannelState = Selectors.getSortedUnreadChannelIds(
      hasMentionMutedChannelState
  ...
  it('get sorted favorite channel ids in current team strict equal', () => {
    ...
    const fromModifiedState = Selectors.getSortedFavoriteChannelIds(
      modifiedState
    );
    assert.ok(fromOriginalState === fromModifiedState);
  ...
  it('get sorted public channel ids in current team strict equal', () => {
    ...
    const fromModifiedState = Selectors.getSortedPublicChannelIds(
      modifiedState
    );
    assert.ok(fromOriginalState === fromModifiedState);
  ...
  it('get sorted private channel ids in current team strict equal', () => {
    ...
    const fromModifiedState = Selectors.getSortedPrivateChannelIds(
      modifiedState
    );
    assert.ok(fromOriginalState === fromModifiedState);
  ...
  it('filters post IDs by the given condition', () => {
    ...
    const filterPostIDsByArchived = Selectors.filterPostIds(
      channel => channel.delete_at !== 0
    );
    const filterPostIDsByUserB = Selectors.filterPostIds(
      (channel, post) => post.user_id === 'b'
    );
    // const filterPostIDsInvalid = Selectors.filterPostIds((channel, post) => {
    //   throw new Error();
    // }); // eslint-disable-line
  ...
  it('isCurrentChannelFavorite', () => {
    ...
    assert.ok(Selectors.isCurrentChannelFavorite(newState as any) === false);
  });
  it('isCurrentChannelMuted', () => {
    ...
    assert.ok(Selectors.isCurrentChannelMuted(newState as any) === true);
  });
  it('isCurrentChannelArchived', () => {
    ...
    assert.ok(Selectors.isCurrentChannelArchived(newState as any) === true);
  });
  it('isCurrentChannelDefault', () => {
    ...
    assert.ok(Selectors.isCurrentChannelDefault(newState as any) === true);
  });
  describe('getDirectAndGroupChannels', () => {
    ...
    it('will return no channels if there is no active user', () => {
      const state = {
        ...
        entities: {
          ...testState.entities,
          users: { ...testState.entities.users, currentUserId: null },
        },
      };
    ...
    it('will return only direct and group message channels', () => {
      const state = {
        ...
        entities: {
          ...testState.entities,
          users: { ...testState.entities.users },
        },
      };
      assert.deepEqual(getDirectAndGroupChannels(state), [
        {
    ...
    it('Should not include deleted users in favorites', () => {
      const newDmChannel: any = TestHelper.fakeDmChannel(user.id, 'newfakeId');
      ...
      const fromOriginalState = Selectors.getSortedFavoriteChannelWithUnreadsIds(
        testState
      );
      const fromModifiedState = Selectors.getSortedFavoriteChannelWithUnreadsIds(
        newState
      );
    ...
    it('get ordered channel ids by_type in current team strict equal', () => {
      ...
      const fromOriginalState = Selectors.getOrderedChannelIds(
        testState,
        null,
        sidebarPrefs.grouping as GroupingType,
        sidebarPrefs.sorting as SortingType,
  ...
  describe('more_direct_channels selector', () => {
    it('getChannelsWithUserProfiles', () => {
      const channelWithUserProfiles = Selectors.getChannelsWithUserProfiles(
        testState
      );
  ...
  describe('get_redirect_channel_name_for_team selector', () => {
    it('getRedirectChannelNameForTeam without advanced permissions', () => {
      const modifiedState = {
        ...
        entities: {
          ...testState.entities,
          general: { ...testState.entities.general, serverVersion: '4.8.0' },
        },
      };
      assert.equal(
    ...
    it('getRedirectChannelNameForTeam with advanced permissions but without JOIN_PUBLIC_CHANNELS permission', () => {
      const modifiedState = {
        ...
        entities: {
          ...
          general: { ...testState.entities.general, serverVersion: '5.12.0' },
        },
      };
      assert.equal(
        Selectors.getRedirectChannelNameForTeam(modifiedState, team1.id),
  ...
  describe('canManageAnyChannelMembersInCurrentTeam', () => {
    it('will return false if channel_user does not have permissions to manage channel members', () => {
      ...
      assert.ok(
        Selectors.canManageAnyChannelMembersInCurrentTeam(newState as any) ===
          false
      );
    });
...
describe('getMyFirstChannelForTeams', () => {
  test('should return the first channel in each team', () => {
    ...
    expect(Selectors.getMyFirstChannelForTeams(state as any)).toEqual({
