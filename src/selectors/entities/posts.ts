...
export const getPostsInCurrentChannel: (
  a: GlobalState
) => Array<PostWithFormatData> | undefined | null = (() => {
...
export function makeGetPostIdsAroundPost(): (
  ...
  return createIdsSelector(
    (state: GlobalState, postId, channelId) =>
    ...
    (
      ...
      let postIds: Array<$ID<Post>> | undefined | null = null;
      let postIndex = -1;
      ...
      const minPostIndex =
        postsAfterCount === -1 ? 0 : Math.max(postIndex - postsAfterCount, 0);
      const maxPostIndex =
        postsBeforeCount === -1
          ? postIds.length
          : Math.min(postIndex + postsBeforeCount + 1, postIds.length); // Needs the extra 1 to include the focused post
...
export function makeGetPostsInChannel(): (
  ...
  return createSelector(
    getAllPosts,
    getPostsInThread,
    (state: GlobalState, channelId: $ID<Channel>) =>
      getPostIdsInChannel(state, channelId),
    getCurrentUser,
    getMyPreferences,
    (state: GlobalState, channelId: $ID<Channel>, numPosts: number) =>
...
export function makeGetPostsAroundPost(): (
  ...
  return createSelector(
    (state: GlobalState, focusedPostId, channelId) =>
    ...
    (
      ...
      for (let i = 0; i < postIds.length; i++) {
        ...
        if (
          shouldFilterJoinLeavePost(post, showJoinLeave, currentUser.username)
        ) {
          continue;
        }
...
export function makeGetCommentCountForPost(): (
  ...
  return createSelector(
    getAllPosts,
    (state, { post }) =>
      state.entities.posts.postsInThread[post ? post.id : ''] || [],
    (state, props) => props,
...
export const getLastPostPerChannel: (
  a: GlobalState
) => RelationOneToOne<Channel, Post> = createSelector(
  getAllPosts,
...
export const getLatestReplyablePostId: (
  a: GlobalState
) => $ID<Post> = createSelector(
...
export const isPostIdSending = (
  ...
  state.entities.posts.pendingPostIds.some(
