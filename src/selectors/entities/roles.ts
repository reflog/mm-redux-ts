...
export const getMyTeamRoles = createSelector(
  (state: GlobalState) => getTeamMemberships(state),
  teamsMemberships => {
    const roles = {};
  ...
  channelsMemberships => {
    ...
    if (channelsMemberships) {
      for (const key in channelsMemberships) {
        if (
          channelsMemberships.hasOwnProperty(key) &&
          channelsMemberships[key].roles
        ) {
...
export const getMyCurrentTeamPermissions = createSelector(
  (state: GlobalState) => getMyTeamRoles(state),
  (state: GlobalState) => getRoles(state),
  (state: GlobalState) => getMySystemPermissions(state),
  (state: GlobalState) => getCurrentTeamId(state),
