...
describe('Selectors.Teams', () => {
  ...
  it('getTeamsList', () => {
    assert.deepEqual(Selectors.getTeamsList(testState), [
      team1,
      team2,
      team3,
      team4,
  ...
  it('getMembersInCurrentTeam', () => {
    assert.deepEqual(
      Selectors.getMembersInCurrentTeam(testState),
      membersInTeam[team1.id]
    );
  });
  it('getTeamMember', () => {
    assert.deepEqual(
      Selectors.getTeamMember(testState, team1.id, user2.id),
      membersInTeam[team1.id][user2.id]
    );
  });
  it('getJoinableTeams', () => {
  ...
  it('getJoinableTeamsUsingPermissions', () => {
    ...
    let modifiedState = {
      entities: {
        ...testState.entities,
        teams: { ...testState.entities.teams, myMembers: {} },
        roles: {
          roles: {
            system_user: {
    ...
    let joinableTeams = Selectors.getJoinableTeams(modifiedState as any);
    ...
    joinableTeams = Selectors.getJoinableTeams(modifiedState as any);
    assert.strictEqual(joinableTeams[0], openTeams[0]);
    assert.strictEqual(joinableTeams[1], openTeams[1]);
    modifiedState = {
      entities: {
  ...
  it('getListableTeamsUsingPermissions', () => {
    ...
    let listableTeams = Selectors.getListableTeams(modifiedState as any);
    ...
    listableTeams = Selectors.getListableTeams(modifiedState as any);
    assert.strictEqual(listableTeams[0], openTeams[0]);
    assert.strictEqual(listableTeams[1], openTeams[1]);
    modifiedState = {
      entities: {
  ...
  it('getMyTeamMember', () => {
    assert.deepEqual(
      Selectors.getMyTeamMember(testState, team1.id),
      myMembers[team1.id]
    );
  });
  it('getTeam', () => {
      ...
      entities: {
        ...
        teams: {
          ...
          teams: {
            ...
            [team3.id]: { ...team3, allow_open_invite: false },
          },
        },
      },
    };
    ...
    const modifiedState = {
      ...
      entities: {
        ...
        teams: {
          ...
          teams: {
            ...testState.entities.teams.teams,
            [team3.id]: { ...team3, display_name: 'Welcome' },
          },
        },
      },
    };
    ...
    const updateState = {
      ...
      entities: {
        ...
        teams: {
          ...
          teams: {
            ...testState.entities.teams.teams,
            [team2.id]: { ...team2, display_name: 'Yankz' },
          },
        },
      },
    };
  ...
  it('getTeamMentions', () => {
    ...
    assert.ok(mentions2 === 3); // Not a member of the team
  ...
  it('getCurrentRelativeTeamUrl', () => {
    assert.deepEqual(
      Selectors.getCurrentRelativeTeamUrl(testState),
      '/' + team1.name
    );
    assert.deepEqual(
  ...
  it('getCurrentTeamUrl', () => {
    ...
    const withSiteURLState = {
      ...testState,
      entities: { ...testState.entities, general },
    };
    withSiteURLState.entities.general = general;
    assert.deepEqual(
      Selectors.getCurrentTeamUrl(withSiteURLState),
      siteURL + '/' + team1.name
    );
    ...
    assert.deepEqual(
      Selectors.getCurrentTeamUrl(withCredentialURLState),
