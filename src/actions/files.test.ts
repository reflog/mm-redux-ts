...
describe('Actions.Files', () => {
  ...
  it('uploadFile', async () => {
    ...
    const testImageData = fs.createReadStream(
      `test/assets/images/${testFileName}`
    );
  ...
  it('getFilesForPost', async () => {
    ...
    const fileUploadResp = await basicClient4.uploadFile(
      imageFormData,
      formBoundary
    );
    ...
    nock(Client4.getPostsRoute())
      .post('')
      .reply(201, {
        ...TestHelper.fakePostWithId(basicChannel.id),
        ...fakePostForFile,
      });
  ...
  it('getMissingFilesForPost', async () => {
    ...
    await Actions.getMissingFilesForPost(postForFile.id)(
      store.dispatch,
      store.getState
    );
    const {
    ...
    assert.ok(fileIdsByPostId[postForFile.id]);
    assert.equal(fileIdsByPostId[postForFile.id][0], fileId);
    const { data: files } = (await Actions.getMissingFilesForPost(
      postForFile.id
