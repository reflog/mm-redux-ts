...
function completeLogin(data: UserProfile): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    try {
      const membersRequest: Promise<
        Array<TeamMembership>
    ...
    if (
      !isMinimumServerVersion(serverVersion, 4, 7) &&
      getConfig(getState()).EnableCustomEmoji === 'true'
    ) {
...
export function loadMe(): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    const serverVersion =
      Client4.getServerVersion() || getState().entities.general.serverVersion;
...
export function getMissingProfilesByUsernames(
...
export function getProfilesInGroupChannels(
...
export function getMe(): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    const me: ActionResult = await getMeFunc(dispatch, getState);
...
export function setStatus(status: { user_id; status }): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    try {
      ...
      if (member) {
        if (member.mention_count > 0 && isDirectChannel(channel)) {
          ...
          if (
            ...
            makeDirectChannelVisibleIfNecessary(otherUserId)(
              dispatch,
              getState
            );
          }
...
export function updateMe(user: Partial<UserProfile>): ActionFunc {
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    dispatch(
      {
...
export function patchUser(user: Partial<UserProfile>): ActionFunc {
