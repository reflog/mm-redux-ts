...
export let systemEmojis: Map<string, any> = new Map();
export function setSystemEmojis(emojis: Map<string, any>) {
  systemEmojis = emojis;
}
...
export function getCustomEmojisByName(names: Array<string>): ActionFunc {
  return async (dispatch, getState) => {
    ...
    names.forEach(name =>
      promises.push(getCustomEmojiByName(name)(dispatch, getState))
    );
...
export function getCustomEmojisInText(text: string): ActionFunc {
  return async (dispatch, getState) => {
    ...
    const emojisToLoad = parseNeededCustomEmojisFromText(
      text,
      systemEmojis,
      customEmojisByName,
      nonExistentEmoji
...
export function getAllCustomEmojis(
  ...
  return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
    ...
    do {
      try {
        let emojis = [];
        emojis = await Client4.getCustomEmojis(
          page,
          perPage,
          Emoji.SORT_BY_NAME
