...
describe('Actions.Teams', () => {
  ...
  it('selectTeam', async () => {
    await Actions.selectTeam(TestHelper.basicTeam)(
      store.dispatch,
      store.getState
    );
  ...
  it('getTeamMembers', async () => {
    ...
    await Actions.getTeamMembers(TestHelper.basicTeam.id)(
      store.dispatch,
      store.getState
    );
    const membersInTeam = store.getState().entities.teams.membersInTeam;
  ...
  it('addUsersToTeam', async () => {
    ...
    await Actions.addUsersToTeam(TestHelper.basicTeam.id, [user.id, user2.id])(
      store.dispatch,
      store.getState
    );
    const members = store.getState().entities.teams.membersInTeam;
  ...
  it('removeUserFromTeam', async () => {
    ...
    nock(Client4.getTeamMemberRoute(TestHelper.basicTeam.id, user.id))
      .delete('')
      .reply(200, OK_RESPONSE);
    await Actions.removeUserFromTeam(TestHelper.basicTeam.id, user.id)(
      store.dispatch,
      store.getState
    );
  ...
  it('setTeamIcon', async () => {
    ...
    const imageData = fs.createReadStream('test/assets/images/test.png') as any;
    nock(Client4.getTeamRoute(team.id))
      .post('/image')
      .reply(200, OK_RESPONSE);
    const { data } = (await Actions.setTeamIcon(team.id, imageData)(
      store.dispatch,
      store.getState
    )) as { data };
    assert.deepEqual(data, OK_RESPONSE);
