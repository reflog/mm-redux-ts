...
import { Channel, ChannelNotifyProps } from 'types/channels';
const OK_RESPONSE = {
  status: 'OK',
};
describe('Actions.Channels', () => {
  ...
  it('createChannel', async () => {
    ...
    assert.equal(
      myMembers[Object.keys(channels)[0]].user_id,
      TestHelper.basicUser.id
    );
  ...
  it('createDirectChannel', async () => {
    ...
    assert.ok(
      channels[Object.keys(myMembers)[0]],
      'channels should have the member'
    );
    assert.ok(
      myMembers[Object.keys(channels)[0]],
    ...
    assert.ok(
      profilesInChannel[created.id],
      'profiles in channel is empty for channel'
    );
    assert.equal(
      profilesInChannel[created.id].size,
      2,
    ...
    assert.ok(
      profilesInChannel[created.id].has(user.id),
  ...
  it('createGroupChannel', async () => {
    ...
    await store.dispatch(
      login(TestHelper.basicUser.email, TestHelper.basicUser.password)
    );
    nock(Client4.getUsersRoute())
      .post('/ids')
    ...
    const result = await store.dispatch(
      Actions.createGroupChannel([TestHelper.basicUser.id, user.id, user2.id])
    );
    ...
    assert.ok(
      profilesInChannel[created.id].has(user2.id),
  ...
  it('convertChannelToPrivate', async () => {
    ...
    await store.dispatch(
      Actions.convertChannelToPrivate(TestHelper.basicChannel.id)
    );
    const updateRequest = store.getState().requests.channels.updateChannel;
  ...
  it('getChannelByNameAndTeamName', async () => {
    nock(Client4.getTeamsRoute())
      .get(
        `/name/${TestHelper.basicTeam.name}/channels/name/${TestHelper.basicChannel.name}?include_deleted=false`
      )
      .reply(200, TestHelper.basicChannel);
    await store.dispatch(
  ...
  it('fetchMyChannelsAndMembers', async () => {
    ...
    await store.dispatch(
      Actions.fetchMyChannelsAndMembers(TestHelper.basicTeam.id)
    );
    const {
      channels,
      channelsInTeam,
  ...
  it('updateChannelNotifyProps', async () => {
    const notifyProps: ChannelNotifyProps = {
      mark_unread: 'mention',
      desktop: 'none',
    };
    nock(Client4.getUsersRoute())
    ...
    nock(Client4.getChannelsRoute())
      .put(
        `/${TestHelper.basicChannel.id}/members/${TestHelper.basicUser.id}/notify_props`
      )
  ...
  it('viewChannel', async () => {
    ...
    nock(Client4.getChannelsRoute())
      ...
      .reply(200, OK_RESPONSE);
    await store.dispatch(
      Actions.viewChannel(TestHelper.basicChannel.id, userChannel.id)
    );
  });
  it('markChannelAsViewed', async () => {
    ...
    await store.dispatch(
      Actions.markChannelAsViewed(TestHelper.basicChannel.id)
    );
  ...
  describe('markChannelAsUnread', () => {
    it('plain message', async () => {
      ...
      store.dispatch(
        Actions.markChannelAsUnread(teamId, channelId, [
  ...
  it('searchAllChannels', async () => {
    ...
    const { data } = await store.dispatch(
      Actions.searchAllChannels('test', '0')
    );
    const moreRequest = store.getState().requests.channels.getAllChannels;
  ...
  it('getChannelMembers', async () => {
    ...
    assert.ok(
      membersInChannel[TestHelper.basicChannel.id][TestHelper.basicUser.id]
    );
  });
  ...
  it('addChannelMember', async () => {
    ...
    assert.ok(
      stats[channelId].member_count >= 1,
    ...
    assert.ok(stats[channelId].member_count, 'member count for channel');
    assert.ok(
      stats[channelId].member_count >= 2,
      'incorrect member count for channel'
    );
  });
  ...
  it('leaveChannel', done => {
    async function test() {
      ...
      nock(
        ...
        .delete('')
        .reply(200, OK_RESPONSE); // This action will retry after 1000ms
  ...
  it('leave private channel', async () => {
    ...
    nock(Client4.getChannelsRoute())
      .post('')
      .reply(201, {
        ...TestHelper.fakeChannelWithId(TestHelper.basicTeam.id),
        ...newChannel,
      });
    const { data: channel } = await store.dispatch(
      Actions.createChannel(newChannel, TestHelper.basicUser.id)
    );
    let channels = store.getState().entities.channels.channels;
    assert.ok(channels[channel.id]);
    nock(Client4.getChannelsRoute())
      .delete(
        `/${TestHelper.basicChannel.id}/members/${TestHelper.basicUser.id}`
      )
      .reply(200, OK_RESPONSE);
  ...
  it('joinChannelByName', async () => {
    ...
    nock(Client4.getTeamsRoute())
      .get(
        `/${TestHelper.basicTeam.id}/channels/name/${secondChannel.name}?include_deleted=true`
      )
  ...
  it('autocompleteChannels', async () => {
    ...
    nock(Client4.getTeamRoute(TestHelper.basicChannel.team_id))
      ...
      .reply(200, [TestHelper.basicChannel]);
    const result = await store.dispatch(
      Actions.autocompleteChannels(TestHelper.basicChannel.team_id, prefix)
    );
  ...
  it('updateChannelScheme', async () => {
    TestHelper.mockLogin();
    await store.dispatch(
      login(TestHelper.basicChannelMember.email, 'password1')
    );
    nock(Client4.getChannelsRoute())
      .post('')
      .reply(201, TestHelper.fakeChannelWithId(TestHelper.basicTeam.id));
  ...
  it('updateChannelMemberSchemeRoles', async () => {
    ...
    nock(Client4.getChannelsRoute())
      ...
      .reply(200, OK_RESPONSE);
    await store.dispatch(
      Actions.updateChannelMemberSchemeRoles(channelId, userId, true, true)
    );
    const update1 = store.getState().entities.channels.membersInChannel[
      channelId
    ][userId];
    assert.ok(update1);
    ...
    nock(Client4.getChannelsRoute())
      ...
      .reply(200, OK_RESPONSE);
    await store.dispatch(
      Actions.updateChannelMemberSchemeRoles(channelId, userId, false, false)
    );
    const update2 = store.getState().entities.channels.membersInChannel[
      channelId
    ][userId];
    assert.ok(update2);
  ...
  it('membersMinusGroupMembers', async () => {
    ...
    const { error } = (await Actions.membersMinusGroupMembers(
      ...
      page,
      perPage
    )(store.dispatch, store.getState)) as { error };
