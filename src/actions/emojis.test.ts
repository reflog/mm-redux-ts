...
describe('Actions.Emojis', () => {
  ...
  it('getAllCustomEmojis', async () => {
    ...
    nock(Client4.getEmojisRoute())
      .delete(`/${created2.id}`)
      .reply(200, OK_RESPONSE); // Should have all emojis minus the deleted one
  ...
  it('searchCustomEmojis', async () => {
    ...
    nock(Client4.getEmojisRoute())
      .post('/search')
      .reply(200, [created]);
    await Actions.searchCustomEmojis(created.name, {
      prefix_only: true,
    ...
    const { data } = (await Actions.searchCustomEmojis(created.name, {
      prefix_only: true,
    })(store.dispatch, store.getState)) as { data };
  ...
  it('autocompleteCustomEmojis', async () => {
    ...
    assert.ok(emojis[created.id]);
    Client4.serverVersion = '4.6.0';
    const { data } = (await Actions.autocompleteCustomEmojis(created.name)(
      store.dispatch,
      store.getState
    )) as { data };
    assert.ok(data.length === 0);
  ...
  it('getCustomEmojiByName', async () => {
    ...
    Client4.serverVersion = '4.6.0';
    const { data } = (await Actions.getCustomEmojiByName(created.name)(
      store.dispatch,
      store.getState
    )) as { data };
  ...
  it('getCustomEmojisByName', async () => {
    ...
    await Actions.getCustomEmojisByName([created.name, missingName])(
      store.dispatch,
      store.getState
    );
    const state = store.getState();
