...
describe('Actions.Admin', () => {
  ...
  it('getComplianceReport', async () => {
    ...
    if (request.status === RequestStatus.FAILURE) {
      throw new Error(
        'getComplianceReport request failed err=' + request.error
      );
    }
  ...
  it('getComplianceReports', async () => {
    ...
    if (request.status === RequestStatus.FAILURE) {
      throw new Error(
        'getComplianceReports request failed err=' + request.error
      );
    }
  ...
  it('getSamlCertificateStatus', async () => {
    ...
    if (request.status === RequestStatus.FAILURE) {
      throw new Error(
        'getSamlCertificateStatus request failed err=' + request.error
      );
    }
  ...
  it('uploadPublicSamlCertificate', async () => {
    ...
    if (request.status === RequestStatus.FAILURE) {
      throw new Error(
        'uploadPublicSamlCertificate request failed err=' + request.error
      );
    }
  });
  it('uploadPrivateSamlCertificate', async () => {
    ...
    if (request.status === RequestStatus.FAILURE) {
      throw new Error(
        'uploadPrivateSamlCertificate request failed err=' + request.error
      );
    }
  });
  it('uploadIdpSamlCertificate', async () => {
    ...
    if (request.status === RequestStatus.FAILURE) {
      throw new Error(
        'uploadIdpSamlCertificate request failed err=' + request.error
      );
    }
  });
  it('removePublicSamlCertificate', async () => {
    ...
    if (request.status === RequestStatus.FAILURE) {
      throw new Error(
        'removePublicSamlCertificate request failed err=' + request.error
      );
    }
  });
  it('removePrivateSamlCertificate', async () => {
    ...
    if (request.status === RequestStatus.FAILURE) {
      throw new Error(
        'removePrivateSamlCertificate request failed err=' + request.error
      );
    }
  });
  it('removeIdpSamlCertificate', async () => {
    ...
    if (request.status === RequestStatus.FAILURE) {
      throw new Error(
        'removeIdpSamlCertificate request failed err=' + request.error
      );
    }
  });
  ...
  it('purgeElasticsearchIndexes', async () => {
    ...
    if (request.status === RequestStatus.FAILURE) {
      throw new Error(
        'purgeElasticsearchIndexes request failed err=' + request.error
      );
    }
  });
  ...
  it('getStandardAnalytics', async () => {
    ...
    assert.ok(
      teamAnalytics[TestHelper.basicTeam.id][Stats.TOTAL_PUBLIC_CHANNELS] > 0
    );
  });
  it('getAdvancedAnalytics', async () => {
    nock(Client4.getBaseRoute())
      ...
      .reply(200, [
        {
          name: 'file_post_count',
          value: 24,
        },
        {
          name: 'hashtag_post_count',
          value: 876,
        },
        {
          name: 'incoming_webhook_count',
          value: 16,
        },
        {
          name: 'outgoing_webhook_count',
          value: 18,
        },
        {
          name: 'command_count',
  ...
  it('getUsersPerDayAnalytics', async () => {
    ...
    assert.ok(teamAnalytics[TestHelper.basicTeam.id]);
    assert.ok(
      teamAnalytics[TestHelper.basicTeam.id][Stats.USERS_WITH_POSTS_PER_DAY]
    );
  });
  it('overwritePlugin', async () => {
    const data1 = fs.createReadStream('test/setup.ts');
    const data2 = fs.createReadStream('test/setup.ts');
    const testPlugin = {
      id: 'testplugin',
      webapp: {
        bundle_path: '/static/somebundle.js',
      },
    };
    nock(Client4.getBaseRoute())
      .post('/plugins', body => {
    ...
    await Actions.uploadPlugin(data1 as any, false)(
      store.dispatch,
      store.getState
    );
    nock(Client4.getBaseRoute())
      ...
      .reply(200, testPlugin);
    await Actions.uploadPlugin(data2 as any, true)(
      store.dispatch,
      store.getState
    );
    const state = store.getState();
  ...
  it('getLdapGroups is_linked', async () => {
    ...
    await Actions.getLdapGroups(0, 100, {
      q: '',
      is_linked: true,
