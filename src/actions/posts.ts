...
import { Action, ActionResult, batchActions } from 'types/actions';
import { FileInfo } from 'types/files';
import { Post } from 'types/posts'; // receivedPost should be dispatched after a single post from the server. This typically happens when an existing post
// is updated.
...
export function createPost(post, files: FileInfo[] = []) {
  return async (dispatch, getState) => {
    ...
    if (files.length) {
      ...
      newPost = { ...newPost, file_ids: fileIds };
      dispatch({
...
export function createPostImmediately(post, files: FileInfo[] = []) {
  return async (dispatch, getState) => {
    const state = getState();
    const currentUserId = state.entities.users.currentUserId;
    const timestamp = Date.now();
...
export function resetCreatePostRequest() {
  return {
    type: PostTypes.CREATE_POST_RESET_REQUEST,
  };
}
...
export function pinPost(postId) {
  return async (dispatch, getState) => {
    ...
    if (post) {
      actions.push(
        receivedPost({ ...post, is_pinned: true, update_at: Date.now() })
      );
    }
...
export function unpinPost(postId) {
  return async (dispatch, getState) => {
    ...
    if (post) {
      actions.push(
        receivedPost({ ...post, is_pinned: false, update_at: Date.now() })
      );
    }
  ...
  return async (dispatch, getState) => {
    ...
    try {
      ...
      dispatch(
        batchActions([
          {
            type: PostTypes.GET_POST_THREAD_FAILURE,
            error,
          },
          logError(error),
        ]),
...
export function getPostsAround(
  ...
  return async (dispatch, getState) => {
    ...
    const posts = {
      posts: { ...after.posts, ...thread.posts, ...before.posts },
      order: [
...
export function getProfilesAndStatusesForPosts(
  ...
  const state = getState();
  const { currentUserId, profiles, statuses } = state.entities.users; // Statuses and profiles of the users who made the posts
  ...
  if (usernamesToLoad.size > 0) {
    promises.push(
      getProfilesByUsernames(Array.from(usernamesToLoad))(dispatch, getState)
    );
  ...
  if (emojisToLoad && emojisToLoad.size > 0) {
    promises.push(
      getCustomEmojisByName(Array.from(emojisToLoad))(dispatch, getState)
    );
  }
  ...
  posts.forEach(post => {
    if (post.message.includes(':')) {
      ...
      if (emojisFromPost.size > 0) {
        customEmojisToLoad = new Set<string>([
          ...customEmojisToLoad,
          ...emojisFromPost,
        ]);
      }
    ...
    if (props && props.attachments && props.attachments.length) {
      ...
      if (attachmentText) {
        ...
        if (emojisFromAttachment.size > 0) {
          customEmojisToLoad = new Set([
            ...customEmojisToLoad,
            ...emojisFromAttachment,
          ]);
        }
      }
    }
...
export function handleNewPost(msg) {
  return async (dispatch, getState) => {
    ...
    if (myChannelMember && Object.keys(myChannelMember).length === 0) {
      //TODO: WTF?  && myChannelMember.constructor === 'Object'
...
function lastPostActions(post, websocketMessageProps) {
  return async (dispatch, getState) => {
    ...
    if (markAsRead) {
      dispatch(
        markChannelAsRead(post.channel_id, undefined, markAsReadOnServer)
      );
