...
describe('Actions.Preferences', () => {
  ...
  it('savePrefrences', async () => {
    ...
    nock(Client4.getUsersRoute())
      .put(`/${TestHelper.basicUser.id}/preferences`)
      .reply(200, OK_RESPONSE);
    await Actions.savePreferences(user.id, preferences)(
      store.dispatch,
      store.getState
    );
    const state = store.getState();
  ...
  it('makeDirectChannelVisibleIfNecessary', async () => {
    ...
    await login(user.email, 'password1')(store.dispatch, store.getState); // Test that a new preference is created if none exists
    ...
    nock(Client4.getUsersRoute())
      .put(`/${TestHelper.basicUser.id}/preferences`)
      .reply(200, OK_RESPONSE);
    Actions.savePreferences(user.id, [{ ...preference, value: 'false' }])(
      store.dispatch,
      store.getState
    );
    nock(Client4.getUsersRoute())
  ...
  it('saveTheme', async () => {
    ...
    assert.ok(
      myPreferences[`theme--${team.id}`],
      "theme preference doesn't exist"
    );
    assert.deepEqual(
      myPreferences[`theme--${team.id}`].value,
