    ...
    if (currentTeamId) {
      ...
      if (currentTeamMembership) {
        ...
        if (data && data.members) {
          const stillMemberOfCurrentChannel = data.members.find(
            m => m.channel_id === currentChannelId
          );
...
function handleUserAddedEvent(msg) {
  return (dispatch, getState) => {
    ...
    if (msg.broadcast.channel_id === currentChannelId) {
      dispatch(getChannelStats(teamId)); // TODO: wtf?, currentChannelId
    }
...
function handleUserRemovedEvent(msg) {
  return (dispatch, getState) => {
    ...
    if (msg.broadcast.user_id === currentUserId && currentTeamId) {
      ...
      dispatch(getChannelStats(currentTeamId)); //TODO: wtf , currentChannelId
    }
  };
}
...
function handleChannelViewedEvent(msg) {
  return (dispatch, getState) => {
    ...
    if (
      ...
      currentUserId === msg.broadcast.user_id
    ) {
...
function handlePreferencesChangedEvent(msg) {
  return (dispatch, getState) => {
    ...
    preferences.forEach(pref => {
      if (
        pref.category === Preferences.CATEGORY_FLAGGED_POST &&
        !posts[pref.name]
      ) {
...
function getAddedDmUsersIfNecessary(preferences) {
  return (dispatch, getState) => {
    ...
    for (const preference of preferences) {
      if (
        preference.category === Preferences.CATEGORY_DIRECT_CHANNEL_SHOW &&
