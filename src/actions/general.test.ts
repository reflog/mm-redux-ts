...
describe('Actions.General', () => {
  ...
  it('getClientConfig', async () => {
    ...
    const clientConfig = store.getState().entities.general.config; // Check a few basic fields since they may change over time
  ...
  it('getLicenseConfig', async () => {
    ...
    const licenseConfig = store.getState().entities.general.license; // Check a few basic fields since they may change over time
  ...
  describe('getRedirectLocation', () => {
    it('new server', async () => {
      ...
      let requestStatus = store.getState().requests.general.redirectLocation
        .status;
      assert.equal(requestStatus, RequestStatus.NOT_STARTED); // Should have followed the link

      const result = await store.dispatch(
        Actions.getRedirectLocation('http://examp.le')
      );
    ...
    it('old server', async () => {
      ...
      let requestStatus = store.getState().requests.general.redirectLocation
        .status;
      assert.equal(requestStatus, RequestStatus.NOT_STARTED); // Should return the original link

      const result = await store.dispatch(
        Actions.getRedirectLocation('http://examp.le')
      );
      assert.deepEqual(result.data, {
        location: 'http://examp.le',
      });
      requestStatus = store.getState().requests.general.redirectLocation.status;
      assert.equal(requestStatus, RequestStatus.SUCCESS); // Should not call the API on an old server
    ...
    it('should save the correct location', async () => {
      ...
      nock(Client4.getBaseRoute())
        ...
        .reply(200, '{"location": "https://example.com"}'); // Save the found URL if it finds one

      await store.dispatch(Actions.getRedirectLocation('http://examp.le'));
      const existingURL = store.getState().entities.posts.expandedURLs[
        'http://examp.le'
      ];
      assert.equal(existingURL, 'https://example.com'); // Save the found URL if it finds one

      await store.dispatch(
        Actions.getRedirectLocation('http://nonexisting.url')
      );
      const nonexistingURL = store.getState().entities.posts.expandedURLs[
        'http://nonexisting.url'
      ];
