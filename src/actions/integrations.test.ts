...
describe('Actions.Integrations', () => {
  ...
  it('getCommands', async () => {
    ...
    const noSystemCommands = store.getState().entities.integrations
      .systemCommands;
    assert.equal(
    ...
    const executableCommands = store.getState().entities.integrations
      .executableCommands;
  ...
  it('getAutocompleteCommands', async () => {
    ...
    const request = store.getState().requests.integrations
    ...
    const teamCommands = store.getState().entities.integrations.commands;
    const systemCommands = store.getState().entities.integrations
      .systemCommands;
  ...
  it('executeCommand', async () => {
    ...
    nock(`${Client4.getCommandsRoute()}/execute`)
      .post('')
      .reply(200, []);
    //TODO: the following test seems wrong
