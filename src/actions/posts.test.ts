...
describe('Actions.Posts', () => {
  ...
  it('createPost', async () => {
    ...
    const { posts, postsInChannel } = state.entities.posts as IDMappedObjects<
      Post
    >;
    assert.ok(posts);
    ...
    assert.ok(found, 'failed to find new post in posts'); // postsInChannel[channelId] should not exist as create post should not add entry to postsInChannel when it did not exist before
  ...
  it('createPost with file attachments', async () => {
    ...
    for (const storedPost of Object.values(state.entities.posts
      .posts as IDMappedObjects<Post>)) {
      if (storedPost.message === post.message) {
        newPost = storedPost;
        break;
      }
  ...
  it('deletePostWithReaction', async () => {
    ...
    await login(TestHelper.basicUser.email, 'password1')(
      store.dispatch,
      store.getState
    );
    nock(Client4.getPostsRoute())
  ...
  it('getPostsSince', async () => {
    ...
    const postList = {
      ...
      posts: {
        post0,
        post1,
        // Pretend post1 has been updated
        post3,
        post4,
      },
    };
  ...
  it('getPostsAround', async () => {
    ...
    const {
      ...
      postsInChannel,
      postsInThread,
    } = store.getState().entities.posts; // should store all of the posts
    ...
    expect(posts).toHaveProperty('root'); // should only store the posts that we know the order of
  ...
  it('removeReaction', async () => {
    ...
    nock(Client4.getUsersRoute())
      .delete(
        `/${TestHelper.basicUser.id}/posts/${post1.id}/reactions/${emojiName}`
      )
      .reply(200, OK_RESPONSE);
  ...
  it('getCustomEmojiForReaction', async () => {
    ...
    const { data: created } = (await createCustomEmoji(
      ...
      testImageData
    )(store.dispatch, store.getState)) as { data };
    nock(Client4.getEmojisRoute())
  ...
  it('resetHistoryIndex', async () => {
    ...
    await Actions.addMessageIntoHistory('test2')(dispatch, getState);
    await Actions.addMessageIntoHistory('test3')(dispatch, getState);
    let index = getState().entities.posts.messagesHistory.index[
      Posts.MESSAGE_TYPES.POST
    ];
    assert.ok(index === 3);
    index = getState().entities.posts.messagesHistory.index[
      Posts.MESSAGE_TYPES.COMMENT
    ];
    assert.ok(index === 3);
    await Actions.moveHistoryIndexBack(Posts.MESSAGE_TYPES.POST)(
      dispatch,
      getState
    );
    await Actions.moveHistoryIndexBack(Posts.MESSAGE_TYPES.POST)(
    ...
    assert.ok(index === 2);
    await Actions.resetHistoryIndex(Posts.MESSAGE_TYPES.POST)(
      dispatch,
      getState
    );
    index = getState().entities.posts.messagesHistory.index[
    ...
    assert.ok(index === 2);
    await Actions.resetHistoryIndex(Posts.MESSAGE_TYPES.COMMENT)(
      dispatch,
      getState
    );
    index = getState().entities.posts.messagesHistory.index[
  ...
  describe('getProfilesAndStatusesForPosts', () => {
    describe('different values for posts argument', () => {
      ...
      it('null', async () => {
        await Actions.getProfilesAndStatusesForPosts(
          undefined,
          dispatch,
          getState
        );
