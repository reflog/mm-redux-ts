...
describe('Actions.Search', () => {
  ...
  it('Perform Search', async () => {
    ...
    post2 = await Client4.createPost(post2); // Test for a couple of words
    ...
    await Actions.searchPosts(TestHelper.basicTeam.id, search1)(
      dispatch,
      getState
    );
    ...
    const searchIsPresent = recent[TestHelper.basicTeam.id].findIndex(
      r => r.terms === search1
    );
    ...
    assert.ok(current.isEnd); // DISABLED
