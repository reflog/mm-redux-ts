...
describe('Actions.Websocket', () => {
  ...
  beforeAll(async () => {
    ...
    const connUrl = (Client4.getUrl() + '/api/v4/websocket').replace(
      /^http:/,
      'ws:'
    );
    mockServer = new Server(connUrl);
    return store.dispatch(Actions.init('web', null, null, MockWebSocket));
  });
  afterAll(async () => {
    ...
    mockServer.emit(
      ...
      JSON.stringify({
        ...
        data: {
          ...
          channel_type: 'O',
          post: `{"id": "71k8gz5ompbpfkrzaxzodffj8w", "create_at": 1508245311774, "update_at": 1508245311774, "edit_at": 0, "delete_at": 0, "is_pinned": false, "user_id": "${TestHelper.basicUser.id}", "channel_id": "${channelId}", "root_id": "", "parent_id": "", "original_id": "", "message": "Unit Test", "type": "", "props": {}, "hashtags": "", "pending_post_id": "t36kso9nwtdhbm8dbkd6g4eeby: 1508245311749"}`,
  ...
  it('Websocket Handle Post Edited', async () => {
    const post = {
      id: '71k8gz5ompbpfkrzaxzodffj8w',
    };
    mockServer.emit(
      'message',
      JSON.stringify({
        event: WebsocketEvents.POST_EDITED,
        data: {
          post: `{"id": "71k8gz5ompbpfkrzaxzodffj8w","create_at": 1508245311774,"update_at": 1508247709215,"edit_at": 1508247709215,"delete_at": 0,"is_pinned": false,"user_id": "${TestHelper.basicUser.id}","channel_id": "${TestHelper.basicChannel.id}","root_id": "","parent_id": "","original_id": "","message": "Unit Test (edited)","type": "","props": {},"hashtags": "","pending_post_id": ""}`,
        },
        broadcast: {
          omit_users: null,
          user_id: '',
          channel_id: '18k9ffsuci8xxm7ak68zfdyrce',
          team_id: '',
        },
        seq: 2,
      })
  ...
  it('Websocket Handle Post Deleted', async () => {
    ...
    mockServer.emit(
      'message',
      JSON.stringify({
        event: WebsocketEvents.POST_DELETED,
        data: {
          post: `{"id": "71k8gz5ompbpfkrzaxzodffj8w","create_at": 1508245311774,"update_at": 1508247709215,"edit_at": 1508247709215,"delete_at": 0,"is_pinned": false,"user_id": "${TestHelper.basicUser.id}","channel_id": "${post.channel_id}","root_id": "","parent_id": "","original_id": "","message": "Unit Test","type": "","props": {},"hashtags": "","pending_post_id": ""}`,
        },
        broadcast: {
          omit_users: null,
          user_id: '',
  ...
  it('Websocket Handle Reaction Added to Post', done => {
    async function test() {
      const emoji = '+1';
      const post = {
        id: 'w7yo9377zbfi9mgiq5gbfpn3ha',
      };
      mockServer.emit(
        'message',
        JSON.stringify({
          event: WebsocketEvents.REACTION_ADDED,
          data: {
            reaction: `{"user_id":"${TestHelper.basicUser.id}","post_id":"w7yo9377zbfi9mgiq5gbfpn3ha","emoji_name":"${emoji}","create_at":1508249125852}`,
          },
          broadcast: {
            omit_users: null,
            user_id: '',
      ...
      setTimeout(() => {
        ...
        assert.ok(
          reactionsForPost.hasOwnProperty(`${TestHelper.basicUser.id}-${emoji}`)
        );
        done();
      }, 500);
    }
  ...
  it('Websocket Handle Reaction Removed from Post', done => {
    async function test() {
      ...
      mockServer.emit(
        'message',
        JSON.stringify({
          event: WebsocketEvents.REACTION_REMOVED,
          data: {
            reaction: `{"user_id":"${TestHelper.basicUser.id}","post_id":"w7yo9377zbfi9mgiq5gbfpn3ha","emoji_name":"+1","create_at":0}`,
          },
          broadcast: {
            omit_users: null,
            user_id: '',
      ...
      function checkForRemove() {
        return new Promise(resolve => {
          setTimeout(() => {
            ...
            const reactionsForPost = reactions[post.id];
            assert.ok(
              !reactionsForPost.hasOwnProperty(
                `${TestHelper.basicUser.id}-${emoji}`
              )
            );
            resolve();
            done();
  ...
  it('Websocket handle team updated', done => {
    async function test() {
      const team = {
        id: '55pfercbm7bsmd11p5cjpgsbwr',
      };
      mockServer.emit(
        'message',
        JSON.stringify({
          event: WebsocketEvents.UPDATE_TEAM,
          data: {
            team: `{"id":"55pfercbm7bsmd11p5cjpgsbwr","create_at":1495553950859,"update_at":1508250370054,"delete_at":0,"display_name":"${TestHelper.basicTeam.display_name}","name":"${TestHelper.basicTeam.name}","description":"description","email":"","type":"O","company_name":"","allowed_domains":"","invite_id":"m93f54fu5bfntewp8ctwonw19w","allow_open_invite":true}`,
          },
          broadcast: {
            omit_users: null,
            user_id: '',
  ...
  it('WebSocket Leave Team', async () => {
    ...
    store.dispatch({
      ...
      data: TestHelper.basicUser,
    });
    store.dispatch({
      type: TeamTypes.RECEIVED_TEAM,
      data: TestHelper.basicTeam,
    });
    store.dispatch({
      type: TeamTypes.RECEIVED_MY_TEAM_MEMBER,
      data: TestHelper.basicTeamMember,
    });
    mockServer.emit(
      'message',
      JSON.stringify({
        event: WebsocketEvents.LEAVE_TEAM,
        data: {
          team_id: team.id,
          user_id: TestHelper.basicUser.id,
        },
  ...
  it('Websocket Handle User Added', async () => {
    ...
    mockServer.emit(
      'message',
      JSON.stringify({
        event: WebsocketEvents.USER_ADDED,
        data: {
          team_id: TestHelper.basicTeam.id,
          user_id: user.id,
        },
  ...
  it('Websocket Handle User Removed', async () => {
    ...
    mockServer.emit(
      'message',
      JSON.stringify({
        event: WebsocketEvents.USER_REMOVED,
        data: {
          remover_id: TestHelper.basicUser.id,
          user_id: user.id,
        },
        broadcast: {
          omit_users: null,
  ...
  it('Websocket Handle User Updated', async () => {
    ...
    mockServer.emit(
      'message',
      JSON.stringify({
        event: WebsocketEvents.USER_UPDATED,
        data: {
          user: {
            id: user.id,
            create_at: 1495570297229,
            update_at: 1508253268652,
            delete_at: 0,
            username: 'tim',
            auth_data: '',
            auth_service: '',
            email: 'tim@bladekick.com',
            nickname: '',
            first_name: 'tester4',
            last_name: '',
            position: '',
  ...
  it('Websocket Handle Channel Created', done => {
    async function test() {
      ...
      mockServer.emit(
        'message',
        JSON.stringify({
          event: WebsocketEvents.CHANNEL_CREATED,
          data: {
            channel_id: '95tpi6f4apy39k6zxuo3msxzhy',
            team_id: TestHelper.basicTeam.id,
          },
          broadcast: {
            omit_users: null,
            user_id: 't36kso9nwtdhbm8dbkd6g4eeby',
            channel_id: '',
            team_id: '',
          },
          seq: 57,
  ...
  it('Websocket Handle Channel Converted', async () => {
    ...
    store.dispatch({
      type: ChannelTypes.RECEIVED_CHANNEL,
      data: { ...TestHelper.basicChannel, type: channelType },
    });
    await TestHelper.wait(300);
    const state = store.getState();
    const entities = state.entities;
  ...
  it('Websocket Handle Channel Deleted', done => {
    async function test() {
      ...
      await store.dispatch(
        ChannelActions.selectChannel(TestHelper.basicChannel.id)
      );
      store.dispatch({
        type: ChannelTypes.RECEIVED_CHANNEL,
        data: {
          ...
          team_id: TestHelper.basicTeam.id,
          display_name: General.DEFAULT_CHANNEL,
        },
      });
      store.dispatch({
        type: ChannelTypes.RECEIVED_CHANNEL,
        data: TestHelper.basicChannel,
      });
  ...
  it('Websocket Handle Direct Channel', done => {
    async function test() {
      const channel = {
        id: TestHelper.generateId(),
        name: TestHelper.basicUser.id + '__' + TestHelper.generateId(),
        type: 'D',
      };
      nock(Client4.getChannelsRoute())
      ...
      mockServer.emit(
        'message',
        JSON.stringify({
          event: WebsocketEvents.DIRECT_ADDED,
          data: {
            teammate_id: 'btaxe5msnpnqurayosn5p8twuw',
          },
          broadcast: {
            omit_users: null,
            user_id: '',
  ...
  it('Websocket handle user added to team', done => {
    async function test() {
      ...
      mockServer.emit(
        'message',
        JSON.stringify({
          event: WebsocketEvents.ADDED_TO_TEAM,
          data: {
            team_id: team.id,
            user_id: TestHelper.basicUser.id,
          },
    ...
    async function test() {
      ...
      mockServer.emit(
        ...
        JSON.stringify({
          ...
          data: {
            emoji: `{"id":"1mmgakhhupfgfm8oug6pooc5no","create_at":1508263941321,"update_at":1508263941321,"delete_at":0,"creator_id":"t36kso9nwtdhbm8dbkd6g4eeby","name":"${TestHelper.generateId()}"}`,
          },
          broadcast: {
            omit_users: null,
            user_id: '',
  ...
  it('handle license changed', done => {
    async function test() {
      mockServer.emit(
        'message',
        JSON.stringify({
          event: WebsocketEvents.LICENSE_CHANGED,
          data: {
            license: {
              IsLicensed: 'true',
            },
  ...
  it('handle config changed', done => {
    async function test() {
      mockServer.emit(
        'message',
        JSON.stringify({
          event: WebsocketEvents.CONFIG_CHANGED,
          data: {
            config: {
  ...
  it('handle open dialog', done => {
    async function test() {
      mockServer.emit(
        'message',
        JSON.stringify({
          event: WebsocketEvents.OPEN_DIALOG,
          data: {
            dialog: JSON.stringify({
              url: 'someurl',
              trigger_id: 'sometriggerid',
              dialog: {},
            }),
          },
        })
...
describe('Actions.Websocket doReconnect', () => {
  ...
  beforeAll(() => {
    (UserActions as any).getStatusesByIds = jest.fn().mockReturnValue({
      type: MOCK_GET_STATUSES_BY_IDS,
    });
    nock(Client4.getBaseRoute())
      .get('/status/ids')
      .reply(200, []);
    (TeamActions as any).getMyTeamUnreads = jest.fn().mockReturnValue({
      type: MOCK_MY_TEAM_UNREADS,
    });
    nock(Client4.getBaseRoute())
      .get('/users/me/teams/unread')
      .reply(200, []);
    (TeamActions as any).getMyTeams = jest.fn().mockReturnValue({
      type: MOCK_GET_MY_TEAMS,
    });
    nock(Client4.getBaseRoute())
      .get('/users/me/teams')
      .reply(200, []);
    (TeamActions as any).getMyTeamMembers = jest.fn().mockReturnValue({
      type: MOCK_GET_MY_TEAM_MEMBERS,
    });
    nock(Client4.getBaseRoute())
      .get(`/users/me/teams/${currentTeamId}/channels/members`)
      .reply(200, []);
    (PostActions as any).getPosts = jest.fn().mockReturnValue({
      type: MOCK_GET_POSTS,
    });
    nock(Client4.getBaseRoute())
      .get(`/channels/${currentChannelId}/posts`)
      .reply(200, []);
    (ChannelActions as any).fetchMyChannelsAndMembers = jest
      .fn()
      .mockReturnValue({
  ...
  it('handle doReconnect after user left current team', async () => {
    ...
    (state.entities.teams.myMembers as any) = {};
    ...
    const expectedActions = [
      ...
      },
      {
        ...
        data: initialState.entities.teams.teams[currentTeamId],
      },
      {
        type: GeneralTypes.WEBSOCKET_SUCCESS,
      },
