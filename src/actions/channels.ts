...
export function deleteChannel(channelId: string): ActionFunc {
  return async (dispatch, getState) => {
    ...
    const viewArchivedChannels =
      state.entities.general.config.ExperimentalViewArchivedChannels === 'true';
...
export function markChannelAsViewed(
  channelId: string,
  prevChannelId: string = ''
): ActionFunc {
  return async (dispatch, getState) => {
...
export function getAllChannelsWithCount(
  ...
  excludeDefaultChannels: boolean = false
): ActionFunc {
  return async (dispatch, getState) => {
    dispatch(
      {
...
export function searchAllChannels(
  ...
  return async (dispatch, getState) => {
    ...
    try {
      channels = await Client4.searchAllChannels(
        term,
        notAssociatedToGroup,
        excludeDefaultChannels
      );
...
export function updateChannelMemberRoles(
  ...
  return async (dispatch, getState) => {
    ...
    const membersInChannel = getState().entities.channels.membersInChannel[
      channelId
    ];
...
export function markChannelAsRead(
  channelId: string,
  prevChannelId?: string,
  updateLastViewedAt: boolean = true
): ActionFunc {
  return async (dispatch, getState) => {
    ...
    const state = getState();
    const { channels, myMembers } = state.entities.channels; // Update channel member objects to set all mentions and posts as viewed
