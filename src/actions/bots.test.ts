...
describe('Actions.Bots', () => {
  ...
  it('disableBot', async () => {
    ...
    nock(Client4.getBotRoute())
      .post('')
      .reply(200, bot);
    await store.dispatch(BotActions.createBot(bot)); // Disable the bot by setting delete_at to a value > 0
  ...
  it('logout', async () => {
    ...
    nock(Client4.getBotRoute())
      .post('')
      .reply(200, bot);
    await store.dispatch(BotActions.createBot(bot)); // Should be cleared by logout
    ...
    await store.dispatch(UserActions.logout()); // Check is clear
