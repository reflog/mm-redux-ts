...
export function handleUploadFilesRequest(
  ...
  switch (action.type) {
    ...
    case FAILURE: {
      ...
      return { ...state, status: RequestStatus.FAILURE, error };
    }

    case CANCEL:
      return { ...state, status: RequestStatus.CANCELLED, error: null };
...
const _moduleExport: (
  a: FilesRequestsStatuses,
  b: GenericAction
) => FilesRequestsStatuses = combineReducers({
  uploadFiles,
});
