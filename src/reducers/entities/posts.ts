...
export function handlePosts(state: { [k: string]: any } = {}, action) {
  switch (action.type) {
...
function handlePostReceived(nextState, post) {
  ...
  if (
    ...
    nextState[post.pending_post_id]
  ) {
...
export function handlePendingPosts(state: any[] = [], action) {
  switch (action.type) {
    case PostTypes.RECEIVED_NEW_POST: {
      const post = action.data;
...
export function postsInChannel<T>(
  state: any = {},
  action,
  prevPosts?,
  nextPosts?
  ...
  switch (action.type) {
    case PostTypes.RECEIVED_NEW_POST: {
      ...
      if (recentBlockIndex === -1) {
        ...
        nextRecentBlock = { ...recentBlock, order: [...recentBlock.order] };
      }
      ...
      return { ...state, [post.channel_id]: nextPostsForChannel };
    }
    ...
    case PostTypes.RECEIVED_POST: {
      ...
      const nextRecentBlock = { ...recentBlock, order: [...recentBlock.order] };
    ...
    case PostTypes.RECEIVED_POSTS_IN_CHANNEL: {
      ...
      if (recent) {
        ...
        if (recentBlockIndex !== -1) {
          ...
          const nextRecentBlock = { ...recentBlock, recent: false };
          nextPostsForChannel[recentBlockIndex] = nextRecentBlock;
        }
      ...
      return { ...state, [action.channelId]: nextPostsForChannel };
    }
    ...
    case PostTypes.RECEIVED_POSTS_SINCE: {
      ...
      const mostOldestCreateAt =
        nextPosts[recentBlock.order[recentBlock.order.length - 1]].create_at;
      const nextRecentBlock = { ...recentBlock, order: [...recentBlock.order] }; // Add any new posts to the most recent block while skipping ones that were only updated
    ...
    case PostTypes.POST_DELETED: {
      ...
      for (let i = 0; i < nextPostsForChannel.length; i++) {
        ...
        const nextOrder = block.order.filter(
          postId => prevPosts[postId].root_id !== post.id
        );

        if (nextOrder.length !== block.order.length) {
          nextPostsForChannel[i] = { ...block, order: nextOrder };
          changed = true;
        }
      }
    ...
    case PostTypes.POST_REMOVED: {
      ...
      for (let i = 0; i < nextPostsForChannel.length; i++) {
        ...
        const nextOrder = block.order.filter(
          postId => postId !== post.id && prevPosts[postId].root_id !== post.id
        );
    ...
    case ChannelTypes.LEAVE_CHANNEL: {
      ...
      const nextState = { ...state };
      Reflect.deleteProperty(nextState as any, channelId);
      return nextState;
    }
...
export function postsInThread<T>(
  state: any = {},
  action: AnyAction,
  prevPosts?
): T {
  switch (action.type) {
    ...
    case PostTypes.RECEIVED_POST: {
      ...
      return { ...state, [post.root_id]: nextPostsForThread };
    }
    ...
    case PostTypes.RECEIVED_POSTS_IN_CHANNEL:
    case PostTypes.RECEIVED_POSTS_SINCE: {
      const newPosts: any[] = Object.values(action.data.posts);
    ...
    case PostTypes.RECEIVED_POSTS_IN_THREAD: {
      ...
      return { ...state, [action.rootId]: nextPostsForThread };
    }
    ...
    case PostTypes.POST_DELETED: {
      ...
      const nextState = { ...state };
      Reflect.deleteProperty(nextState as any, post.id);
      return nextState;
    }
    ...
    case ChannelTypes.LEAVE_CHANNEL: {
      ...
      for (const rootId of Object.keys(state)) {
        if (prevPosts[rootId].channel_id === channelId) {
          Reflect.deleteProperty(nextState as any, rootId);
          postDeleted = true;
        }
      }
...
export function reactions(state = {}, action) {
  switch (action.type) {
    case PostTypes.RECEIVED_REACTIONS: {
      ...
      return { ...state, [action.postId]: nextReactions };
    }
    ...
    case PostTypes.RECEIVED_REACTION: {
      ...
      nextReactions[reaction.user_id + '-' + reaction.emoji_name] = reaction;
      return { ...state, [reaction.post_id]: nextReactions };
    }
...
function storeReactionsForPost(state, post) {
  ...
  return { ...state, [post.id]: reactionsForPost };
}
...
function storeOpenGraphForPost(state, post) {
  ...
  return post.metadata.embeds.reduce((nextState, embed) => {
    ...
    return { ...nextState, [embed.url]: embed.data };
  }, state);
}

function messagesHistory(state: any = {}, action) {
  switch (action.type) {
...
export function expandedURLs(state = {}, action) {
  switch (action.type) {
    case GeneralTypes.REDIRECT_LOCATION_SUCCESS:
      return { ...state, [action.data.url]: action.data.location };

    case GeneralTypes.REDIRECT_LOCATION_FAILURE:
      return { ...state, [action.data.url]: action.data.url };
