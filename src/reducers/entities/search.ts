...
function results(state: any[] = [], action) {
  switch (action.type) {
...
function flagged(state: any[] = [], action) {
  switch (action.type) {
...
function removePinnedPost(state, post) {
  if (post && state[post.channel_id]) {
    ...
    if (index !== -1) {
      ...
      return { ...state, [channelId]: pinnedPosts };
    }
  }
