...
function syncables(state: any = {}, action: Redux.AnyAction) {
  switch (action.type) {
    case GroupTypes.RECEIVED_GROUP_TEAMS: {
      return {
        ...state,
        [action.group_id]: { ...state[action.group_id], teams: action.data },
      };
    }
    ...
    case GroupTypes.RECEIVED_GROUP_CHANNELS: {
      return {
        ...state,
        [action.group_id]: { ...state[action.group_id], channels: action.data },
      };
    }

    case GroupTypes.LINKED_GROUP_TEAM: {
      let nextGroupTeams: any[] = [];
