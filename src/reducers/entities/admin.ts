    ...
    case AdminTypes.RECEIVED_PLUGIN_STATUSES: {
      ...
      for (const plugin of action.data || []) {
        ...
        nextState[id] = {
          ...
          description:
            (nextState[id] && nextState[id].description) || plugin.description,
          version: (nextState[id] && nextState[id].version) || plugin.version,
          is_prepackaged:
            (nextState[id] && nextState[id].is_prepackaged) ||
            Plugins.PREPACKAGED_PLUGINS.includes(id),
...
function ldapGroups(state = {}, action) {
  switch (action.type) {
    ...
    case AdminTypes.LINK_LDAP_GROUP_FAILURE: {
      ...
      if (nextState[action.data]) {
        nextState[action.data] = { ...nextState[action.data], failed: true };
      }
