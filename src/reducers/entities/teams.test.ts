...
describe('Reducers.teams.myMembers', () => {
  it('initial state', async () => {
    let state = {};
    state = teamsReducer(state as any, {} as any);
    assert.deepEqual((state as any).myMembers, {}, 'initial state');
  });
  it('RECEIVED_MY_TEAM_MEMBER', async () => {
    ...
    state = teamsReducer(state as any, testAction);
  ...
  it('RECEIVED_MY_TEAM_MEMBERS', async () => {
    ...
    state = teamsReducer(state as any, testAction);
    assert.deepEqual((state as any).myMembers, testAction.result);
    state = teamsReducer(state as any, {} as any);
    assert.deepEqual((state as any).myMembers, testAction.result);
  });
  it('RECEIVED_TEAMS_LIST', async () => {
    ...
    state = teamsReducer(state as any, testAction);
    assert.deepEqual(state.myMembers, testAction.result); // remove deleted team to teams.myMembers list

    team2.delete_at = 1;
    testAction.data = [team2];
    state = teamsReducer(state as any, testAction);
    assert.deepEqual(state.myMembers, {
      team_id_1: { ...team1, msg_count: 0, mention_count: 0 },
    });
  });
