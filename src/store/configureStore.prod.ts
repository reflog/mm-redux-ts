...
export default function configureOfflineServiceStore(
  preloadedState,
  appReducer,
  userOfflineConfig,
  getAppReducer,
  ...
  const store = createStore(
    ...
    baseState,
    offlineCompose(baseOfflineConfig)(createMiddleware(clientOptions), [])
  );
  reducerRegistry.setChangeListener(reducers => {
    store.replaceReducer(
      createOfflineReducer(createReducer(baseState, reducers))
    );
