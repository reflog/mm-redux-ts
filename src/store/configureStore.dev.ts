...
const devToolsEnhancer =
  typeof window !== 'undefined' && (window as any).__REDUX_DEVTOOLS_EXTENSION__ // eslint-disable-line no-underscore-dangle
...
export default function configureServiceStore(
  ...
  const store = createStore(
    ...
    baseState, // eslint-disable-line - offlineCompose(config)(middleware, other funcs)
  ...
  reducerRegistry.setChangeListener(reducers => {
    store.replaceReducer(
      createOfflineReducer(createDevReducer(baseState, reducers))
    );
  }); // launch store persistor

  if (baseOfflineConfig.persist) {
    baseOfflineConfig.persist(
      store,
      baseOfflineConfig.persistOptions,
      baseOfflineConfig.persistCallback
    );
  }
...
function createDevReducer(baseState: GlobalState, ...reducers) {
