...
import { Command, OAuthApp } from 'types/integrations';
...
class TestHelper {
  ...
  fakeUserWithId = (): Partial<UserProfile> & { id } => {
    return {
      ...this.fakeUser(),
      id: this.generateId(),
  ...
  mockScheme = (): Partial<Scheme> => {
    return {
      name: this.generateId(),
      description: this.generateId(),
      scope: 'channel',
      default_channel_admin_role: undefined,
      default_channel_user_role: undefined,
    };
  };

  mockSchemeWithId = (): Partial<Scheme> & { id } => {
    return {
      ...this.mockScheme(),
      id: this.generateId(),
      create_at: 1507840900004,
  ...
  fakeChannel = (teamId?): Partial<Channel> => {
    const name = this.generateId();
  ...
  fakeChannelWithId = (teamId?): Partial<Channel> & { id } => {
    return {
  ...
  fakePost = (channelId?): Partial<Post> => {
    return {
      channel_id: channelId,
      message: `Unit Test ${this.generateId()}`,
      type: 'system_header_change',
      file_ids: new Array<any>(),
    };
  };

  fakePostWithId = (channelId): Partial<Post> => {
    return {
