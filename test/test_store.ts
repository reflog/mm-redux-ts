...
import { DispatchFunc } from 'types/actions';
...
interface DispatchFuncWithActions extends DispatchFunc {
  actions: any[];
}
export function mockDispatch(dispatch): DispatchFuncWithActions {
  const mocked = action => {
    dispatch(action);
